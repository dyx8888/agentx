...
DEFAULT_COLLABORATION_CHAINS = {
    "new_product_launch": {
        "name": "新品上市协作链",
        "steps": [
            {"agent": "product_selector", "task": "选品确认与利润测算", "review": "recommended"},  # 第一步选品，利润测算影响后续所有决策
            {"agent": "brand_bd", "task": "达人筛选与寄样", "review": "recommended"},  # 依赖选品结果，需要寄样给达人
            {"agent": "visual_designer", "task": "主图与详情页设计", "review": "recommended"},  # 设计依赖产品确认
            {"agent": "content_operation", "task": "短视频脚本+直播脚本", "review": "recommended"},  # 内容依赖设计素材
            {"agent": "smart_ad_delivery", "task": "投放策略与预算", "review": "recommended"},  # 投放依赖内容和达人矩阵
            {"agent": "data_analysis", "task": "全链路数据监控接入", "review": "auto"},  # 最后一步监控接入，自动完成
        ],
    },
    "daily_sales": {
        "name": "日常销售协作链",
        "steps": [
            {"agent": "customer_service", "task": "售前咨询自动回复监控", "review": "auto"},  # 日常监控自动执行，无需人工审核
            {"agent": "warehouse_logistics", "task": "备货库存巡检", "review": "auto"},  # 库存巡检也是自动化的
        ],
    },
    "campaign_preparation": {
        "name": "大促筹备协作链",
        "steps": [
            {"agent": "data_analysis", "task": "历史大促数据复盘", "review": "auto"},  # 数据分析先行，为后续决策提供依据
            {"agent": "product_selector", "task": "大促选品与备货量建议", "review": "recommended"},  # 选品依赖复盘数据
            {"agent": "brand_bd", "task": "达人矩阵预约与锁价", "review": "recommended"},  # 达人预约需提前锁定
            ...
            {"agent": "warehouse_logistics", "task": "备货入仓确认", "review": "recommended"},  # 备货确认是最后一道物理环节
            {"agent": "customer_service", "task": "大促话术准备", "review": "recommended"},  # 话术最后准备，确保参数时效
        ],
    },
    "anomaly_response": {
        "name": "异常响应协作链",
        "steps": [
            {"agent": "data_analysis", "task": "异常检测与归因分析", "review": "auto"},  # 异常检测自动触发
            {"agent": "brand_bd", "task": "评估投放影响", "review": "auto"},  # 影响评估自动执行
            {"agent": "smart_ad_delivery", "task": "止损策略执行", "review": "recommended"},  # 止损涉及资金，推荐审核
            {"agent": "customer_service", "task": "客诉应对准备", "review": "mandatory"},  # 客诉应对必须人工确认，避免机械回复引发舆情
        ],
    },
}
...
class CollaborationEngine:
    ...
    def list_chains(self) -> list[dict]:
        return [{"name": k, "title": v["name"], "step_count": len(v["steps"])}  # 仅返回摘要信息
    ...
    def get_review_level(self, agent_key: str, task_type: str = None) -> str:
        ...
        for entry in agent_def.get("applies_to", []):  # 先检查是否在强制审核白名单中
            ...
            if len(parts) >= 2 and parts[0] == agent_key:
                if task_type and parts[1] == task_type or not task_type:  # 精确匹配或模糊匹配
                    return "mandatory"
    ...
    async def trigger_chain(self, chain_name: str, company_id: int, initiator_agent: str,
                            extra_context: dict = None) -> list[int]:
        ...
        for step in chain["steps"]:
            try:
                ...
                review_level = step.get("review", self.get_review_level(step["agent"]))  # 步骤级审核级别优先于 Agent 默认级别
                ...
                logger.info("collaboration_chain_step_created",
                            chain=chain_name, step=step["agent"], task_id=task_id)
            except Exception as e:
                logger.error("collaboration_chain_step_failed",
                             chain=chain_name, step=step["agent"], error=str(e))

        await self._publish_chain_started(chain_name, company_id, task_ids)  # 通知 WebSocket 链已启动
        return task_ids

    async def on_agent_task_completed(self, agent_key: str, company_id: int, task_id: int,
                                       result_summary: str, agent_output: dict = None):
        """Agent任务完成后的自动回写和通知"""
        try:
            await self._write_to_context_bus(agent_key, company_id, result_summary, agent_output)  # 先回写上下文，确保数据持久化
            await self._notify_websocket(company_id, agent_key, task_id, result_summary)  # 再推送通知，前端可据此更新 UI
            logger.info("collaboration_task_completed_handled", agent=agent_key, task_id=task_id)
        except Exception as e:
            logger.error("collaboration_task_completed_handler_error",
                         agent=agent_key, error=str(e))

    async def _write_to_context_bus(self, agent_key: str, company_id: int,
                                      result_summary: str, agent_output: dict = None):
        ...
        try:
            ...
            logger.warning("context_bus_write_failed", agent=agent_key, error=str(e))  # 上下文写入失败不阻塞主流程

    async def _notify_websocket(self, company_id: int, agent_key: str, task_id: int,
                                  summary: str):
        try:
    ...
    async def _publish_chain_started(self, chain_name: str, company_id: int,
                                       task_ids: list[int]):
        try:
    ...
    async def create_alert(self, company_id: int, alert_type: str, title: str,
                             message: str, severity: str = "warning",
                             related_agents: list[str] = None):
        from app.database import db  # 延迟导入
        try:
            alert_id = db.create_alert(company_id, alert_type, title, message, severity)  # 数据库持久化告警
            from app.ws import ws_manager
            ...
            logger.info("collaboration_alert_created", alert_id=alert_id, type=alert_type,
                        severity=severity)
            return alert_id
        except Exception as e:
    ...
    async def mark_chain_step_completed(self, chain_name: str, company_id: int,
                                          agent_key: str):
