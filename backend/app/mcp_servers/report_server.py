...
async def generate_performance_report_http_endpoint(request: PerformanceReportRequest):
...
async def generate_strategy_suggestion_http_endpoint(request: StrategySuggestionRequest):
    """HTTP wrapper for the MCP tool that preserves tenant context."""
    return generate_strategy_suggestion(
        request.platform,
        request.category,
        company_id=request.company_id,
    )
...
async def generate_performance_report(
    ...
    First tries real platform API. Demo fixtures are blocked in production unless
    ALLOW_PLATFORM_MOCK_FALLBACK=true is explicitly set.
    """
    try:
        ...
        if platform_adapter and platform_adapter.is_available():
            try:
                ...
                performance_data = await platform_adapter.get_campaign_report(kol_name, campaign_id)
                if performance_data:
                    ...
                    return ToolResult.ok(
                        data=result, message="Performance report generated from real API."
                    ).to_json()
            except Exception as e:
                logger.error("report_platform_error", error=str(e))

        if not mock_fallback_enabled():
            logger.warning("report_mock_fallback_blocked", kol=kol_name, campaign=campaign_id)
            return mock_fallback_blocked_result("Campaign performance report")
        ...
        return ToolResult.ok(
            data=result, message="Performance report generated from mock data."
        ).to_json()
    except Exception as e:
...
def _generate_mock_report(kol_name: str, campaign_id: str) -> str:
    ...
    cpc = performance_data["cost"] / (
        performance_data["exposure"] * performance_data["click_rate"] / 100
    )
    cpa = (
        performance_data["cost"] / performance_data["sales_count"]
        if performance_data["sales_count"] > 0
        else 0
    )
...
def generate_strategy_suggestion(
    ...
    """
    try:
        _resolve_company_id(str(company_id) if company_id else "default")
        if not mock_fallback_enabled():
            logger.warning("strategy_mock_fallback_blocked", platform=platform, category=category)
            return mock_fallback_blocked_result("Strategy suggestion")
        ...
        model_gateway = get_model_gateway()
        llm = model_gateway.get_llm("deepseek")
        trending_formats = "\n".join(f"• {item}" for item in category_data["trending_formats"])
...
if __name__ == "__main__":
    ...
    if os.getenv("RUN_AS_HTTP_SERVICE", "false").lower() == "true":
        ...
        async def generate_performance_report_endpoint(request: PerformanceReportRequest):
            return await generate_performance_report_http_endpoint(request)
        ...
        async def generate_strategy_suggestion_endpoint(request: StrategySuggestionRequest):
            return await generate_strategy_suggestion_http_endpoint(request)
