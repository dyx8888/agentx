...
async def check_delivery_status_http_endpoint(request: CheckDeliveryStatusRequest):
    """HTTP wrapper for the MCP tool that preserves tenant context."""
    return check_delivery_status(request.order_id, company_id=request.company_id)


async def generate_arrival_script_http_endpoint(request: GenerateArrivalScriptRequest):
    """HTTP wrapper for the MCP tool that preserves tenant context."""
    return generate_arrival_script(
        request.kol_name,
...
@mcp.tool()
def check_delivery_status(order_id: str, company_id: str | int | None = None) -> str:
    """
    ...
    try:
        ...
        if not mock_fallback_enabled():
            logger.warning("delivery_mock_fallback_blocked", order_id=order_id)
            return mock_fallback_blocked_result("Delivery status")
...
def generate_arrival_script(
    ...
    try:
        ...
        return ToolResult.ok(
            data=response.content, message=f"Arrival script generated for {kol_name}."
        ).to_json()
