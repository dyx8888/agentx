...
async def generate_script_http_endpoint(request: GenerateScriptRequest):
...
def generate_script(
    ...
    try:
        ...
        return ToolResult.ok(
            data=response.content, message=f"Video script generated for {kol_name} on {platform}."
        ).to_json()
...
if __name__ == "__main__":
    ...
    if os.getenv("RUN_AS_HTTP_SERVICE", "false").lower() == "true":
        ...
        async def generate_script_endpoint(request: GenerateScriptRequest):
            return await generate_script_http_endpoint(request)
