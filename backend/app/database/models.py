...
from sqlalchemy.ext.declarative import declarative_base  # 经典声明式基类（非 2.0 风格），提供 metadata 和映射能力
from sqlalchemy.orm import relationship  # ORM 的关系导航，避免手动 JOIN，支持懒加载和反向引用

from app.utils.encryption import decrypt_data, encrypt_data  # 字段级加解密工具，敏感数据（API Key、凭证）入库前加密、出库后解密

Base = declarative_base()  # 全局单例 Base，所有模型继承它，共享 metadata，create_all 一次性创建全部表
...
class EncryptedText(TypeDecorator):
    ...
    def process_bind_param(self, value, dialect):
        ...
        if value is not None:
            return encrypt_data(str(value))  # str() 防御性转换：万一传入数字等非字符串类型也不会崩溃
        return value
...
class User(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)  # 自增整型主键，单表百万级以下性能最优；加 index=True 是因为按 id 查询是最频繁操作
    username = Column(String(50), unique=True, index=True, nullable=False)  # 50 字符足够显示名使用；unique 防止重名混淆；index 加速登录查找
    email = Column(String(100), unique=True, index=True, nullable=True)  # nullable=True 而非 False：某些 OAuth 提供商不返回邮箱，允许为空更灵活
    password_hash = Column(String(255), nullable=False)  # 255 长度：bcrypt/argon2 哈希通常 60-100 字符，留足余量应对未来算法升级
    company_id = Column(Integer, ForeignKey("companies.id"), nullable=True)  # nullable=True：超级管理员可能不属于任何公司
    is_admin = Column(Boolean, default=False, nullable=False)  # 默认不是管理员——最小权限原则，管理员权限需要显式授予
    disabled = Column(Boolean, default=False, nullable=False)  # 软禁用而非删除：保留用户数据用于审计和追溯，同时阻止登录
    created_at = Column(DateTime, default=datetime.utcnow)  # UTC 时间无时区，避免跨地域部署时的时间转换混乱

    # Relationships
    company = relationship("Company", back_populates="users")  # back_populates 实现双向关联：user.company 和 company.users 互相导航
...
class Company(Base):
    ...
    brand_name = Column(String(100), nullable=False)  # 品牌名称可能与公司名不同，如"字节跳动" vs "抖音"
    category = Column(String(50), nullable=False)  # 行业分类，用于 Agent 推荐和数据分析维度
    platforms_json = Column(Text, nullable=False)  # Text 而非 String(固定长度)：各公司对接的平台数量不同，JSON 灵活适应
    platform_credentials = Column(EncryptedText, nullable=True, default=None)  # 第三方平台密钥必须加密存储，防止数据库泄露直接暴露凭证
    llm_api_key = Column(EncryptedText, nullable=True, default=None)  # LLM API Key 同样是高价值敏感数据，加密是安全合规的基本要求
    subscription_status = Column(String(20), default="inactive")  # 默认 inactive——要求主动订阅才能使用，避免未付费即享受服务
...
class Agent(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    company_id = Column(Integer, ForeignKey("companies.id"), nullable=False)  # Agent 属于公司，不允许孤立 Agent 存在
    name = Column(String(100), nullable=False)
    description = Column(Text, nullable=False)  # Text：Agent 的能力描述可能很长，String 有截断风险
    tools_json = Column(Text, nullable=False)  # 工具列表用 JSON 存储，支持无需数据库迁移即可增删工具（热插拔）
    created_at = Column(DateTime, default=datetime.utcnow)
    ...
    source_tasks = relationship("Task", foreign_keys="Task.source_agent_id")  # 必须显式指定 foreign_keys，因为 Task 表还有其他外键指向 Agent 表
...
class Task(Base):
    ...
    source_agent_id = Column(Integer, ForeignKey("agents.id"), nullable=True)  # nullable=True：系统任务可能无源 Agent，如手动创建的任务
    target_agent_name = Column(String(100), nullable=False)  # 目标 Agent 名称，用于跨 Agent 的任务路由
    task_description = Column(Text, nullable=False)
    status = Column(String(20), default="pending", nullable=False)  # 用 String 而非 SQL ENUM：更灵活，新状态无需 ALTER TABLE
    result = Column(Text, nullable=True)  # nullable=True：任务可能在 pending/processing 状态时尚无结果
    created_at = Column(DateTime, default=datetime.utcnow)
    completed_at = Column(DateTime, nullable=True)  # 仅在任务完成时设值，可用于计算任务耗时和 SLA 达标率

    # Relationships
    company = relationship("Company", back_populates="tasks")
    source_agent = relationship("Agent", foreign_keys=[source_agent_id])  # 显式指定外键，避免与 Task 表中其他指向 Agent 的外键混淆
...
class Feedback(Base):
    ...
    original_output = Column(Text, nullable=False)  # 必须保存原始输出，才能对比人工修改后的差异用于训练
    human_edited_output = Column(Text, nullable=True)  # nullable=True：有些反馈可能只是评分，不提供修改后的文本
    status = Column(String(20), default="pending", nullable=False)  # pending 表示未处理，需人工审核后才能用于模型改进
    created_at = Column(DateTime, default=datetime.utcnow)
...
class EvolutionLog(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    agent_id = Column(Integer, ForeignKey("agents.id"), nullable=False)
    tool_name = Column(String(100), nullable=True)  # nullable=True：进化建议可能不针对特定工具，而是通用性改进
    suggestion_text = Column(Text, nullable=False)  # 进化建议内容，不可为空——没有建议内容就没有记录意义
    created_at = Column(DateTime, default=datetime.utcnow)
    applied = Column(Boolean, default=False, nullable=False)  # 默认未应用：改进建议不应自动生效，需人工审核后手动标记
...
class SubscriptionPlan(Base):
    ...
    capabilities = Column(Text, nullable=True)            # JSON 存储能力列表，不同套餐的能力项不同，Text+JSON 比多对多关系表更简洁
    is_active = Column(Boolean, default=True)  # 软删除标志：下架套餐后历史订阅记录仍能找到对应的 Plan
    created_at = Column(DateTime, default=datetime.utcnow)
...
class CompanySubscription(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    company_id = Column(Integer, ForeignKey("companies.id"), nullable=False)
    plan_id = Column(Integer, ForeignKey("subscription_plans.id"), nullable=False)  # 关联到具体套餐，通过 plan 关系可获取价格和能力
    agent_name = Column(String(100), nullable=False)      # 订阅与具体 Agent 绑定：一个公司可能有多个 Agent，各 Agent 可订阅不同套餐
    status = Column(String(20), default="active")         # active/cancelled/expired，用于计费逻辑和权限判断
...
class CompanyAgentTool(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)  # autoincrement 显式声明，确保 SQLite 兼容性
    company_id = Column(Integer, ForeignKey("companies.id"), nullable=False)
    agent_name = Column(String(50), nullable=False)
    tool_name = Column(String(100), nullable=False)
    enabled = Column(Boolean, default=True)  # 默认启用：新工具上线后自动对公司可见，降低手动配置成本
    created_at = Column(DateTime, default=datetime.utcnow)

    __table_args__ = (
        UniqueConstraint('company_id', 'agent_name', 'tool_name', name='uq_company_agent_tool'),  # 三元组唯一：防止同一公司同一 Agent 下重复添加同一工具
    )
...
class CompanyAgentSkill(Base):
    ...
    __table_args__ = (
        UniqueConstraint('company_id', 'agent_name', 'skill_name', name='uq_company_agent_skill'),  # 三元组唯一约束，防止重复配置
    )
...
class TaskBoard(Base):
    ...
    task_description = Column(Text, nullable=False)
    assigned_agent = Column(String(50), nullable=False)  # 字符串引用 Agent 名而非 FK：支持 Agent 被删除后任务仍然可追溯
    status = Column(String(20), default="pending", nullable=False)  # String 而非 ENUM：新增状态无需 DDL 变更
    priority = Column(Integer, default=2, nullable=False)  # 默认中等优先级(2/5)——避免所有任务都是最高/最低，中位默认值最合理
    parent_task_id = Column(Integer, ForeignKey("task_board.id"), nullable=True)  # 自引用外键支持父子任务拆分
    result_summary = Column(Text, nullable=True)
    created_at = Column(DateTime, default=datetime.utcnow)
    updated_at = Column(DateTime, default=datetime.utcnow)  # 独立于 created_at，支持按最近修改时间排序

    # Relationships
    parent_task = relationship("TaskBoard", remote_side=[id])  # remote_side 必须指定：自引用关系需要告诉 ORM "远端"是哪一侧（父任务端）

    def __repr__(self):
        return f"<TaskBoard(id={self.id}, project_name='{self.project_name}', status='{self.status}')>"
...
class ResultCache(Base):
    ...
    agent_name = Column(String(50), nullable=False)
    task_type = Column(String(50), nullable=False)  # 任务类型用于缓存键匹配：不同任务类型即便输入相同，输出语义也不同
    input_summary = Column(Text, nullable=False)  # 输入摘要作为缓存键，需要不可为空——没有输入就没有缓存的匹配依据
...
class DecisionLog(Base):
    ...
    options_considered = Column(Text, nullable=False)  # JSON 格式备选方案列表，保留完整上下文用于审计
    ...
    outcome = Column(String(20), default="pending", nullable=False)  # pending/success/failed——用于回测决策质量
    created_at = Column(DateTime, default=datetime.utcnow)
...
class FeedbackLog(Base):
    ...
    rating = Column(Integer, nullable=True)  # 1-5 李克特量表，用于量化满意度；nullable=True 表示可不评分
...
class SkillEvolutionLog(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)
    skill_name = Column(String(100), nullable=False)
    version = Column(String(20), nullable=False)  # 语义化版本号，如 "1.2.0"，比纯数字更能表达变更程度
    ...
    applied = Column(Boolean, default=False, nullable=False)  # 默认未应用：改进不应自动生效，需人工审核
...
class UserBehavior(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)
    user_id = Column(Integer, nullable=True)  # nullable=True：匿名用户（未登录访客）的行为也需要记录
    company_id = Column(Integer, ForeignKey("companies.id"), nullable=False)
    action_type = Column(String(50), nullable=False)  # 如 chat/tool_call/feedback——枚举值用于分组统计
...
class UserLora(Base):
    ...
    base_model = Column(String(100), nullable=False)  # 基座模型名，如 "qwen-7b"——LoRA 必须与基座匹配才能加载
    training_samples_count = Column(Integer, default=0, nullable=False)  # 0 表示尚未训练，非 0 表示训练样本数，用于评估模型质量
    last_trained_at = Column(DateTime, nullable=True)  # nullable=True：可能从未被训练过
    is_active = Column(Boolean, default=True, nullable=False)  # 一个用户可能有多个 LoRA，但只有一个是当前激活的
...
class A2AMessage(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)
    message_id = Column(String(64), unique=True, nullable=False)  # 64 字符唯一 ID（类 UUID），unique 约束保证幂等性
    sender_agent_name = Column(String(100), nullable=False)
    recipient_agent_name = Column(Text, nullable=False)  # Text 而非 String：接收方可能是逗号分隔的多个 Agent 名称，长度不可预测
    task_description = Column(Text, nullable=False)
