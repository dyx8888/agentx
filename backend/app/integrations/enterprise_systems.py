...
class EnterpriseIntegrationUnavailable(RuntimeError):
    """Raised when production config would otherwise use mock enterprise data."""
...
def _enterprise_mock_enabled() -> bool:
    env = os.getenv("ENV", "development").strip().lower()
    if env not in {"prod", "production"}:
        return True
    return _env_flag("ALLOW_ENTERPRISE_MOCK_INTEGRATIONS", "false")


class ProductionMockIntegrationMixin:
    integration_name = "enterprise"

    def _mock_available(self) -> bool:
        return _enterprise_mock_enabled()

    def _guard_mock(self, operation: str) -> None:
        if _enterprise_mock_enabled():
            return
        logger.warning(
            "enterprise_mock_integration_blocked",
            integration=self.integration_name,
            operation=operation,
        )
        raise EnterpriseIntegrationUnavailable(
            f"{self.integration_name} integration is not configured; "
            f"mock operation '{operation}' is disabled in production"
        )
...
class WMSIntegration(ABC):
    ...
    @abstractmethod
    def create_outbound_order(self, order_id: str, warehouse_id: str, items: list[dict]) -> dict:
        """创建出库单"""
        ...
...
class MockERPIntegration(ProductionMockIntegrationMixin, ERPIntegration):
    ...
    def get_inventory(self, sku: str = None, warehouse: str = None) -> list[dict]:
        self._guard_mock("get_inventory")
    ...
    def get_purchase_orders(self, status: str = None, limit: int = 20) -> list[dict]:
        self._guard_mock("get_purchase_orders")
        return [
            {
                "po_id": "PO20260501",
                "supplier": "供应商A",
    ...
    def create_purchase_order(self, supplier: str, items: list[dict]) -> dict:
        self._guard_mock("create_purchase_order")
        logger.info("erp_mock_create_po", supplier=supplier, items_count=len(items))
        return {
            "po_id": f"PO{hash(str(items)) % 10000:05d}",
            "status": "待审核",
            "supplier": supplier,
            "items": len(items),
        }
    ...
    def get_suppliers(self, category: str = None) -> list[dict]:
        self._guard_mock("get_suppliers")
        return [
...
class MockWMSIntegration(ProductionMockIntegrationMixin, WMSIntegration):
    integration_name = "wms"

    def is_available(self) -> bool:
        return self._mock_available()

    def get_warehouse_list(self) -> list[dict]:
        self._guard_mock("get_warehouse_list")
        return [
    ...
    def get_stock_detail(self, warehouse_id: str, sku: str = None) -> list[dict]:
        self._guard_mock("get_stock_detail")
        return [
    ...
    def create_outbound_order(self, order_id: str, warehouse_id: str, items: list[dict]) -> dict:
        self._guard_mock("create_outbound_order")
        logger.info("wms_mock_create_outbound", order=order_id)
        return {
            "outbound_id": f"OUT{hash(order_id) % 100000:05d}",
            "status": "待拣货",
            "warehouse": warehouse_id,
        }

    def get_picking_list(self, warehouse_id: str = None) -> list[dict]:
        self._guard_mock("get_picking_list")
        return [
            {
                "outbound_id": "OUT00001",
                "order_id": "ORD001",
                "items": 3,
                "warehouse": "WH001",
                "priority": "高",
    ...
    def get_inventory_alerts(self, warehouse_id: str = None) -> list[dict]:
        self._guard_mock("get_inventory_alerts")
        return [
            {
                "sku": "SKU003",
                "current": 8,
            ...
            {"sku": "SKU012", "current": 15, "safe": 30, "alert": "库存预警", "warehouse": "WH002"},
        ]
...
class CustomerServiceIntegration(ABC):
    ...
    @abstractmethod
    def send_reply(self, conversation_id: str, content: str, auto_send: bool = False) -> dict: ...
    @abstractmethod
    def get_customer_profile(self, customer_id: str) -> dict: ...


class MockCustomerServiceIntegration(ProductionMockIntegrationMixin, CustomerServiceIntegration):
    integration_name = "customer_service"

    def get_unread_messages(self, platform: str = None, limit: int = 20) -> list[dict]:
        self._guard_mock("get_unread_messages")
        return [
            {
                "conv_id": "CONV001",
                "customer": "王女士",
    ...
    def get_customer_profile(self, customer_id: str) -> dict:
        self._guard_mock("get_customer_profile")
        return {
