...
MAX_SEMANTIC_MEMORIES = int(
    os.getenv("MAX_SEMANTIC_MEMORIES", "10000")
)  # 长期记忆硬上限，防止 Milvus/Redis 存储无限膨胀
LRU_CONFIDENCE_PROTECT_THRESHOLD = (
    0.9  # 高置信度记忆受保护不被 LRU 淘汰，避免丢失已充分验证的可靠知识
)
SLEEP_CONSOLIDATION_INTERVAL_SEC = int(
    os.getenv("SLEEP_CONSOLIDATION_INTERVAL_SEC", "1800")
)  # 30分钟最小间隔，避免频繁巩固消耗计算资源
SHORT_TERM_CONSOLIDATION_THRESHOLD = int(
...
DECAY_ATTENUATION_DAYS = 60  # 60天未访问 + 低关联度 → 权重衰减
DECAY_PURGE_THRESHOLD = 0.1  # 衰减后分数 < 0.1 → 物理删除
DECAY_LAMBDA = 0.05  # 衰减系数 (比基础的 0.099 更温和，约 14 天半衰期)
DECAY_HIGH_ACCESS_THRESHOLD = 10  # 访问次数 > 10 → 高频保护
DECAY_RECENT_ACCESS_DAYS = 7  # 7天内有访问 → 不受衰减影响
DECAY_ASSOCIATION_WEAK_THRESHOLD = 0.3  # 关联度 < 0.3 → 弱关联，加速衰减
...
class SemanticMemory:
    ...
    # 智能衰减扩展字段
    access_count: int = 0  # 累计访问次数，用于高频保护判断
    last_access: str = field(default_factory=lambda: datetime.utcnow().isoformat())  # 最后访问时间
    association_score: float = 0.5  # 关联活跃度 (0.0~1.0)，每次与其他记忆共同被检索时递增
    decay_status: str = "active"  # 衰减状态: active / dormant / decaying / purged


class DecayStatus(_Enum):
    """记忆衰减状态枚举"""

    ACTIVE = "active"  # 活跃：正常参与检索
    DORMANT = "dormant"  # 休眠：从活跃检索中排除，但仍保留
    DECAYING = "decaying"  # 衰减中：权重持续下降
    PURGED = "purged"  # 已清除：标记为待物理删除
...
class ThreeLayerMemoryManager:
    ...
    _singleton_lock = threading.Lock()  # 保护 __new__ 的 check-then-act，避免并发首调创建多实例

    def __new__(cls):  # 重写 __new__ 而非 __init__ 实现单例，因为 __new__ 控制对象创建
        # 快速路径：已存在实例直接返回，避免热路径加锁开销
        if cls._instance is not None:
            return cls._instance
        # 慢路径：加锁创建，防止并发首调产生多实例
    ...
    def __init__(self):
        if (
            self._initialized
        ):  # 防止重复初始化，因为单例的 __init__ 每次 ThreeLayerMemoryManager() 都会调用
            return
        self._initialized = True
        self._redis = None  # 惰性初始化失败时设为 None，后续操作通过 None 检查优雅降级
        self._milvus = None  # 同上，Milvus 失败不影响 Redis 功能，实现存储层解耦
        self._milvus_connected = (
            False  # 预初始化为 False，防止 _init_milvus 异常时下游 AttributeError
        )
        self._embedding_service = (
            None  # 缓存 EmbeddingService 单例，避免每次 _get_embedding 重建实例
        )
        self._sleep_scheduled = False  # 防止多次调度睡眠巩固任务，避免并发冲突
        self._consolidation_threshold = 50  # 情景记忆超过 50 条触发巩固，确保有足够数据提取模式
        self._last_sleep_time: dict[
            str, float
        ] = {}  # 按公司 ID 记录上次巩固时间，支持多租户独立触发
        self._consolidation_lock = (
            threading.Lock()
    ...
    def _init_milvus(self):
        try:
            ...
            logger.warning(
                "memory_milvus_unavailable", error=str(e)
            )  # warning 而非 error，语义记忆层可降级为仅 Redis
            self._milvus = None
    ...
    def store_episodic(self, memory: EpisodicMemory) -> str:
        ...
        if self._redis:  # Redis 不可用时跳过，不阻塞 PostgreSQL 写入
            try:
                ...
                logger.warning(
                    "episodic_redis_store_failed", error=str(e)
                )  # Redis 失败不抛异常，PostgreSQL 写入继续

        try:
            from app.database import db  # 延迟导入，避免数据库模块未就绪时影响 Redis 写入

            db.create_memory_entry(
                company_id=int(memory.company_id)
                if memory.company_id.isdigit()
                else 1,  # 非数字 company_id 回退为 1（默认公司）
                agent_key=memory.agent_key,
                memory_type="episodic",  # 明确标记记忆类型，方便后续按类型查询和统计
                content=json.dumps(
                    memory_data, ensure_ascii=False
                ),  # 以 JSON 文本存储，保持与 Redis 一致
                memory_id=episode_id,
            )
        except Exception as e:
            logger.debug(
                "episodic_pg_store_skipped", error=str(e)
            )  # debug 级别，因为 PostgreSQL 存储是辅助性的

        logger.debug(
            "episodic_stored", episode_id=episode_id, agent=memory.agent_key
        )  # debug 级别，减少生产日志量
    ...
    def store_semantic(
        ...
        memory_id = hashlib.md5(
            f"{company_id}{content}{category}".encode(), usedforsecurity=False
        ).hexdigest()[:16]  # 相同内容+分类产生相同 ID，天然去重
        ...
        if self._redis:  # Redis 存储语义记忆元数据，作为 Milvus 的元数据缓存层
            try:
                ...
                self._redis.setex(
                    key, 86400 * 30, json.dumps(semantic_data, ensure_ascii=False)
                )  # 30天 TTL，比情景记忆更长的保留期
                # LRU 访问时间追踪
        ...
        if embedding and self._milvus_connected:  # 同时检查嵌入和连接，两个条件都满足才写入向量库
            try:
                ...
                logger.warning(
                    "milvus_insert_failed", error=str(e)
                )  # Milvus 写入失败不影响 Redis 元数据，系统可降级

        logger.debug("semantic_stored", memory_id=memory_id, category=category)
        return memory_id

    def retrieve_episodic(
        self, company_id: str, agent_key: str = None, limit: int = 10
    ) -> list[dict]:
        """检索情景记忆"""
            ...
            try:
                ...
                for eid in episode_ids:
                    data = self._redis.get(
                        f"memory:episodic:{company_id}:{eid}"
                    )  # 逐个获取详情，zset 只存了 ID 索引
                    if data:
                        mem = json.loads(data)
                        if (
                            agent_key is None or mem.get("agent") == agent_key
                        ):  # 在内存中过滤 Agent，因为 zset 没有子索引
                            memories.append(mem)
            except Exception:
    ...
    def retrieve_semantic(
        self, query: str, company_id: str, top_k: int = 5, category: str = None
    ) -> list[dict]:
        """检索语义记忆，并更新 LRU 访问时间"""
        if (
            not self._milvus_connected or self._milvus is None
        ):
            return []

        try:
            if not self._milvus.has_collection(collection_name="agent_memory"):
                logger.debug("semantic_memory_collection_missing")
                return []
            ...
            self._milvus.load_collection("agent_memory", timeout=3)

            # 转义 company_id 中的特殊字符，防止 Milvus 表达式注入
            # company_id 来自调用方，可能包含双引号或反斜杠，破坏 expr 语法或绕过租户隔离
            safe_company_id = company_id.replace("\\", "\\\\").replace('"', '\\"')
            safe_category = category.replace("\\", "\\\\").replace('"', '\\"') if category else None
            filter_expr = f'company_id == "{safe_company_id}"'
            if safe_category:
                filter_expr = f'{filter_expr} && category == "{safe_category}"'
            search_params = {
                "metric_type": "IP",
                "params": {"nprobe": 10},
            }  # 内积（IP）相似度，比 L2 更适合未归一化的嵌入；nprobe=10 平衡精度和速度
            results = self._milvus.search(
                ...
                limit=top_k,
                filter=filter_expr,  # 在公司范围内搜索，实现多租户隔离
                    ...
                    memories.append(
                        {
                            ...
                            "content": entity.get(
                                "content", ""
                            ),  # 从 Milvus entity 获取存储的内容
                            "category": entity.get("category", ""),
                            "score": hit.get("distance", hit.get("score")),  # 相似度分数，可用于排序和过滤
                        }
                    )
                    # 更新 LRU 访问时间
    ...
    def retrieve_few_shot_examples(
        self, agent_key: str, task_type: str, company_id: str, limit: int = 3
    ) -> list[dict]:
        ...
        episodes = self.retrieve_episodic(
            company_id, agent_key, limit=limit
        )  # 回退：从原始情景记忆提取
        examples = []
        for ep in episodes:
    ...
    async def sleep_consolidation(self, company_id: str) -> dict:
        ...
        if not self._consolidation_lock.acquire(
            blocking=False
        ):  # 非阻塞获取锁，已有巩固任务在运行则直接返回
        ...
        try:
            memories = self.retrieve_episodic(
                company_id, limit=100
            )  # 一次最多处理 100 条，避免 LLM 调用超时
            ...
            try:
                ...
                optimized_rules = await self._extract_prompt_rules(
                    memories, company_id
                )  # 同时提取 Prompt 优化规则
    ...
    async def _extract_patterns(self, memories: list[dict], company_id: str) -> list[dict]:
        ...
        try:
            from langchain_core.messages import (  # 使用 LangChain 的标准消息类型，兼容多种 LLM 后端
                HumanMessage,
                SystemMessage,
            )
            ...
            from app.services.model_gateway import (
                get_global_model_gateway,  # 通过网关统一管理 LLM 实例，支持多模型切换
            )
...
如果样本量不足以提炼可靠模式，请按 META_EMPTY_GUIDANCE 格式返回。"""  # 截断至 3000 字符，为系统 Prompt 留出空间
            ...
            # 优先使用 ainvoke 异步调用，避免在 async 函数中阻塞事件循环
            # 兼容老版本 LangChain：若 LLM 对象无 ainvoke，则用 asyncio.to_thread 包装同步调用
            messages = [
        ...
        return [  # LLM 提取失败时的兜底策略：生成一条低置信度聚合摘要
            {
                "content": f"基于{len(memories)}条记忆的聚合摘要",
                "category": "general",
                "source_count": len(memories),
                "confidence": 0.7,
            }  # 兜底置信度 0.7 < 0.9，不享受 LRU 保护
        ]

    async def _extract_prompt_rules(self, memories: list[dict], company_id: str) -> list[str]:
        """提取Prompt优化规则（使用结构化Meta Prompt，含规则级别和触发条件）"""
        if len(memories) < 10:  # 比模式提取的阈值（5）更高，因为规则提取需要更多样本来验证重复性
            return []

        try:
            from langchain_core.messages import (  # 使用 LangChain 标准消息类型
                HumanMessage,
                SystemMessage,
            )
...
如果样本不足以提炼可靠规则，返回空列表。"""
    ...
    def _get_embedding(self, text: str) -> list[float] | None:
        # 复用全局单例，避免每次调用都重建 EmbeddingService（内部含 Redis 连接、本地模型等重资源）
        # EmbeddingService 已通过 get_embedding_service 工厂实现线程安全单例
        if self._embedding_service is None:
            try:
                from app.rag.embedding_service import (
                    get_embedding_service,  # 延迟导入，避免 RAG 模块未就绪时影响记忆管理器初始化
                )
                ...
                return None  # 嵌入服务不可用时返回 None，调用方有 None 检查保护
        try:
            vec = self._embedding_service.encode_single(text)  # 返回 np.ndarray
            if vec is None:  # encode_single 可能返回 None（如降级失败）
                return None
            return vec.tolist()  # tolist() 转 Python 列表，比 numpy array 更易序列化和传输
        except Exception:
            return None  # 嵌入失败返回 None，调用方有 None 检查保护

    def _insert_to_milvus(
        self, memory_id: str, content: str, embedding: list[float], company_id: str, category: str
    ):
        try:
            ...
            self._milvus.flush("agent_memory")  # 手动 flush 确保数据持久化到磁盘，避免断电丢失
        except Exception:
    ...
    def _trigger_consolidation(self, company_id: str):
        ...
        """
        now = time.time()
        last = self._last_sleep_time.get(
            company_id, 0
        )  # 首次触发时 last=0，time_elapsed 会很大，立即触发
        time_elapsed = now - last

        # 时间触发
        time_triggered = (
            time_elapsed >= SLEEP_CONSOLIDATION_INTERVAL_SEC
        )  # 环境变量控制间隔，方便不同环境调整
        ...
        if self._redis:  # Redis 不可用时只能依赖时间触发
            try:
                ...
                volume_triggered = (
                    count >= SHORT_TERM_CONSOLIDATION_THRESHOLD
                )  # 超过环境变量阈值则触发
            except Exception:
        ...
        logger.info(
            "sleep_consolidation_triggered", company=company_id, reason=",".join(trigger_reason)
        )
        ...
        if self._redis:  # Redis 不可用时 LRU 淘汰无法工作，直接跳过
            try:
                ...
                self._redis.zadd(  # zadd 相同元素会更新 score，利用这一特性实现访问时间更新
                    f"memory:semantic_lru:{company_id}",
                    {memory_id: now},  # score=当前时间戳，数值越大表示最近使用
                )
                # 更新访问次数和最后访问时间
                ...
                if data:
                    ...
                    # 访问时从休眠状态恢复
                    if mem.get("decay_status") == "dormant":
                        mem["decay_status"] = "active"
                        mem["confidence"] = min(mem.get("confidence", 0.8) + 0.1, 1.0)
                        logger.info("memory_reactivated_from_dormant", memory_id=memory_id)
                    self._redis.setex(mem_key, 86400 * 30, json.dumps(mem, ensure_ascii=False))
            except Exception:
    ...
    def _evict_lru_semantic(self, company_id: str):
        ...
        try:
            ...
            evict_count = max(
                int((total - self._lru_max_capacity) * 1.1), 1
            )  # 多淘汰 10%，减少频繁淘汰的开销
            evict_count = min(
                evict_count, 1000
            )  # 单次淘汰上限 1000，防止一次性淘汰过多导致性能抖动

            # 从最旧到最新遍历
            evicted = 0
            protected = 0
            batch = self._redis.zrange(
                lru_key, 0, evict_count * 2 - 1, withscores=True
            )  # 扫描 2 倍于目标数量，因为部分会被保护跳过
            ...
            for memory_id, _score in batch:
                ...
                if data:
                    try:
                        mem = json.loads(data)
                        if (
                            mem.get("confidence", 1.0) >= LRU_CONFIDENCE_PROTECT_THRESHOLD
                        ):  # 高置信度记忆受保护，跳过淘汰
                            protected += 1
                            continue  # 不淘汰此条，继续遍历
            ...
            logger.info(
                "lru_semantic_eviction",
                company=company_id,
                evicted=evicted,
                protected=protected,  # 记录保护数量，用于监控是否有过多保护记忆
                remaining=self._redis.zcard(lru_key),
            )  # 记录剩余数量，验证淘汰效果
        except Exception as e:
            logger.warning("lru_eviction_failed", error=str(e))

    # ============ 智能衰减引擎 ============
    # 参考 MemoryBear 遗忘引擎，实现三阶段衰减模型
    # 阶段1: 休眠 (30天未访问) → 阶段2: 衰减 (60天未访问+低关联) → 阶段3: 清除 (分数<0.1)

    def execute_smart_decay_cycle(self, company_id: str) -> dict:
        """执行智能衰减周期 — 遗忘引擎核心

        三阶段衰减:
        1. 休眠检测: 超过 DECAY_DORMANT_DAYS 天未访问 → 标记为休眠
        2. 衰减计算: 超过 DECAY_ATTENUATION_DAYS 天未访问 + 低关联度 → 权重指数衰减
        3. 清除: 衰减后分数 < DECAY_PURGE_THRESHOLD → 物理删除

        保护机制:
        - 高频访问 (> DECAY_HIGH_ACCESS_THRESHOLD 次) + 高置信度 (>0.9) → 永久保护
        - 近 DECAY_RECENT_ACCESS_DAYS 天内有访问 → 不受衰减影响
        - 关联度 > 0.7 → 衰减速度减半

        Returns:
            {"dormant": int, "decayed": int, "purged": int, "protected": int}
        """
        if not self._redis:
            return {
                "dormant": 0,
        ...
        result = {"dormant": 0, "decayed": 0, "purged": 0, "protected": 0}
        ...
        try:
            ...
            for memory_id in all_ids:
                ...
                association = mem.get("association_score", 0.5)
                decay_status = mem.get("decay_status", "active")
                ...
                # 永久保护: 高频访问(>10次) + 高置信度(>0.9)
                if access_count > DECAY_HIGH_ACCESS_THRESHOLD and confidence >= 0.9:
                    result["protected"] += 1
                    continue

                # 近期保护: 7天内有访问
                if days_since_access <= DECAY_RECENT_ACCESS_DAYS:
                    result["protected"] += 1
                    continue

                # --- 阶段1: 休眠检测 ---
                if decay_status == "active" and days_since_access >= DECAY_DORMANT_DAYS:
                    mem["decay_status"] = "dormant"
                    mem["last_updated"] = now_iso
                    self._redis.setex(mem_key, 86400 * 30, json.dumps(mem, ensure_ascii=False))
                    result["dormant"] += 1
                    logger.debug(
                        "memory_entered_dormant",
                        memory_id=memory_id,
                ...
                # --- 阶段2: 衰减计算 ---
                if (
                    decay_status in ("dormant", "decaying")
                    and days_since_access >= DECAY_ATTENUATION_DAYS
                ):
                    # 计算衰减系数
                    decay_factor = 2.71828 ** (-DECAY_LAMBDA * days_since_access)

                    # 关联度修正: 高关联度记忆衰减更慢
                    if association >= 0.7:
                        decay_factor = decay_factor**0.5  # 平方根，衰减速度减半
                    elif association <= DECAY_ASSOCIATION_WEAK_THRESHOLD:
                        decay_factor = decay_factor**2.0  # 平方，衰减速度加倍

                    new_confidence = confidence * decay_factor
                    mem["confidence"] = round(new_confidence, 4)
                    mem["decay_status"] = "decaying"
                    mem["last_updated"] = now_iso
                    self._redis.setex(mem_key, 86400 * 30, json.dumps(mem, ensure_ascii=False))
                    result["decayed"] += 1
                    logger.debug(
                    ...
                    # --- 阶段3: 清除 ---
                    if new_confidence < DECAY_PURGE_THRESHOLD:
                        mem["decay_status"] = "purged"
                        self._redis.setex(mem_key, 86400 * 7, json.dumps(mem, ensure_ascii=False))
                        # 宽限期 7 天后再物理删除（下次 vacuum 清理）
                        result["purged"] += 1
                        logger.info(
                            "memory_purged",
                            memory_id=memory_id,
            ...
            logger.warning("smart_decay_cycle_failed", error=str(e))
            return result

    def _calculate_decay_score(
        self, confidence: float, days_since_access: int, association_score: float = 0.5
    ) -> float:
        """计算智能衰减后的分数

        公式: score = confidence × e^(-λ × days) × association_modifier

        Args:
            confidence: 原始置信度
            days_since_access: 距上次访问的天数
            association_score: 关联活跃度 (0.0~1.0)
        ...
        elif association_score <= DECAY_ASSOCIATION_WEAK_THRESHOLD:
            decay_factor = decay_factor**2.0

        return max(confidence * decay_factor, DECAY_PURGE_THRESHOLD)

    def get_decay_status_summary(self, company_id: str) -> dict:
        """获取记忆衰减状态摘要（用于监控 Dashboard）

        Returns:
            {"active": int, "dormant": int, "decaying": int, "purged": int, "total": int}
        """
        if not self._redis:
            return {"active": 0, "dormant": 0, "decaying": 0, "purged": 0, "total": 0}

        summary = {"active": 0, "dormant": 0, "decaying": 0, "purged": 0, "total": 0}
        try:
            lru_key = f"memory:semantic_lru:{company_id}"
            all_ids = self._redis.zrange(lru_key, 0, -1)
            ...
            for memory_id in all_ids:
                ...
                if data:
                    try:
                        mem = json.loads(data)
                        status = mem.get("decay_status", "active")
    ...
    def calculate_weight_decay(self, memory: SemanticMemory, current_time: str = None) -> float:
    ...
    def resolve_conflict(
        ...
        merged_confidence = memory_a.confidence * weight_a + memory_b.confidence * weight_b

        # 取内容更完整的记忆
        merged_content = (
            memory_a.content if len(memory_a.content) >= len(memory_b.content) else memory_b.content
        )
    ...
    def detect_and_resolve_conflicts(self, company_id: str, category: str = None) -> int:
    ...
    async def vacuum_expired_memories(self, company_id: str, max_age_days: int = 90) -> dict:
        """周期性 Vacuum 清理 - 文档依据: 3.docx

        清理过期记忆和低质量记忆：
        1. 删除超过max_age_days天未更新的记忆
        2. 删除权重衰减后分数低于WEIGHT_DECAY_MIN_SCORE的记忆
        3. 物理删除已标记为 purged 的记忆（宽限期过后）

        Returns:
            {"expired_removed": int, "low_quality_removed": int, "purged_removed": int}
        """
        expired_removed = 0
        ...
        if not self._redis:
            return {"expired_removed": 0, "low_quality_removed": 0, "purged_removed": 0}
        ...
        try:
            ...
            for memory_id in all_ids:
                ...
                should_remove = False
                removal_reason = None  # 记录删除原因，分别统计而非全部计入 expired_removed

                # 优先清理已标记为 purged 的记忆
                if mem.get("decay_status") == "purged":
                    should_remove = True
                    removal_reason = "purged"
                ...
                if not should_remove:
                    ...
                    if decayed <= self.WEIGHT_DECAY_MIN_SCORE:
                        should_remove = True
                        removal_reason = "low_quality"

                if should_remove:
                    self._redis.delete(mem_key)
                    self._redis.zrem(lru_key, memory_id)
                    # 按 reason 分别计数，修复原 expired_removed 重复计数导致字段语义错误的问题
                    if removal_reason == "purged":
                        purged_removed += 1
                    elif removal_reason == "low_quality":
                        low_quality_removed += 1
                    else:  # expired 或解析失败兜底
                        expired_removed += 1

            logger.info(
                "vacuum_cleanup_complete",
                company=company_id,
                expired_removed=expired_removed,
                low_quality_removed=low_quality_removed,
                purged_removed=purged_removed,
            )
            return {
                "expired_removed": expired_removed,
                "low_quality_removed": low_quality_removed,
    ...
    def get_agent_context_injection(
        self, company_id: str, agent_key: str, task_description: str
    ) -> str:
        """为Agent构建上下文注入文本"""
        parts = []  # 用列表收集各部分再 join，比字符串拼接效率高且可读性好

        episodes = self.retrieve_episodic(
            company_id, agent_key, limit=3
        )  # 3 条情景记忆，平衡信息量和上下文长度
        if episodes:  # 有情景记忆才添加此部分，避免空标题
            parts.append("## 近期相关任务记录")
            for i, ep in enumerate(episodes):
                parts.append(
                    f"{i + 1}. {ep.get('summary', '')[:150]}"
                )  # 截断 150 字符，情境记忆在上下文中只需概要
                if ep.get("outcome"):
                    parts.append(f"   结果: {ep['outcome']}")

        semantic = self.retrieve_semantic(
            task_description, company_id, top_k=3
        )  # 用任务描述作为查询，搜索相关经验
        ...
        few_shot = self.retrieve_few_shot_examples(
            agent_key, "general", company_id, limit=2
        )  # 2 个示例，Few-shot 范式下 2-shot 通常足够
