...
def planner_node(state: State, llm, skill_registry, memory_manager, mcp_tools) -> dict[str, Any]:
    ...
    if reflection_feedback:
        ...
        logger.info(
            "planner_receiving_reflection_feedback", feedback_count=len(reflection_feedback)
        )
    ...
    system_prompt = _build_planner_prompt_with_memory(
        skill_content, full_context, reflection_feedback
    )
...
def _build_planner_prompt_with_memory(
    skill_content: str, full_context, reflection_feedback: list = None
) -> str:
...
def _parse_plan(content: str) -> dict:
    ...
    return {
        "steps": [
            {
                "id": 1,
                "description": "Planner output could not be parsed; stop before execution.",
                "expected_output": "planner_parse_error",
            }
        ],
        "acceptance_criteria": ["Planner JSON output must be parseable before execution"],
        "task_summary": content[:100],
        "parse_error": "planner_llm_json_parse_failed",
    }
...
def _validate_plan(plan: dict, user_message: str, available_tools: list) -> list[str]:
    ...
    if plan.get("parse_error"):
        errors.append(plan["parse_error"])
        return errors
    ...
    if len(uncovered) == len(user_requirements) and len(steps) <= 1:
        errors.append(
            f"计划可能未充分覆盖用户需求: 用户消息 '{user_message[:80]}' 未在步骤描述中体现"
        )
