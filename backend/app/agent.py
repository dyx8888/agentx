...
from app.core.instruction_boundary import wrap_system_instructions  # 用分隔标记包裹指令，防止 prompt 注入
...
def get_agent_by_name(name: str):
    ...
    try:
        ...
        # ---- getattr 按命名约定动态获取函数：避免为每个 Agent 写 if/elif 分支，新增 Agent 只需遵循约定 ----
        system_prompt = getattr(module, f"get_{name}_system_prompt")()
        default_tools = getattr(module, f"get_{name}_default_tools")()
...
async def create_agent(
    ...
    logger.info("agent_created", mode=mode_label, tool_count=len(all_tools), has_skill=bool(skill_content))
    return agent
...
def create_agent_sync(
    ...
    raise RuntimeError(
        "create_agent_sync cannot be called from within an async context. "
