...
"""  # 客服引擎：关键词情绪识别 + 三级发送决策；生产默认不自动外发，高风险自动升级
...
def _customer_auto_send_enabled() -> bool:
...
class CustomerServiceEngine:
    ...
    async def process_message(
        self, message: CustomerMessage, company_id: int
    ) -> ReplyDraft:  # 核心处理方法：情绪识别→生成回复→计算置信度→决定发送策略
        emotion = self._detect_emotion(
            message.content
        )  # 先做情绪识别，因为高风险消息的回复策略完全不同

        try:
            reply_content = await self._generate_reply(
                message, company_id
            )  # 调用LLM生成回复，失败时回退到规则回复
            confidence = self._calculate_confidence(
                reply_content, emotion
            )  # 综合回复质量和情绪计算置信度
            ...
            await self._handle_emotion_escalation(
                message, emotion, company_id
            )  # 高风险消息触发升级告警

            logger.info(
                "cs_message_processed",
                conv=message.conv_id,
                emotion=emotion.value,
                decision=send_decision.value,
            )  # 结构化日志便于追踪
            return draft
    ...
    def _detect_emotion(
        self, content: str
    ) -> EmotionLevel:  # 基于关键词的情绪检测，先匹配高风险再匹配中风险
    ...
    async def _generate_reply(
        self, message: CustomerMessage, company_id: int
    ) -> str:  # 优先使用LLM生成回复，失败时回退到规则回复
        try:
            from langchain_core.messages import (  # LangChain消息类型，用于LLM调用
                HumanMessage,
                SystemMessage,
            )
            ...
            from app.core.instruction_boundary import (
                wrap_system_instructions,  # 包装系统指令，防止注入
            )
            ...
            response = llm.invoke(
                [  # 同步调用LLM（与fastapi异步配合使用）
                    SystemMessage(
                        content=wrap_system_instructions(system_prompt)
                    ),  # 系统消息包装后发送
                    HumanMessage(
                        content=(  # 人类消息包含完整的上下文信息
                            f"请为以下客服消息生成回复:\n"
            ...
            return self._rule_based_reply(
                message.content
            )  # LLM调用失败时回退到规则回复，保证服务可用性

    def _rule_based_reply(
        self, content: str
    ) -> str:  # 基于关键词的规则回复，作为LLM失败时的兜底策略
        reply_map = {  # 关键词→回复模板映射，覆盖最常见的客服场景
            "发货": "亲，我会先为您生成发货状态查询草稿；需要订单系统确认后，客服才能告知实际发货进度。",
            "物流": "亲，我会先为您生成物流查询草稿；需要物流系统返回结果后，客服才能告知实际轨迹。",
            "退款": "亲，我已记录您的退款诉求草稿；退款需按订单和售后政策人工审核后处理，当前不会承诺已受理或已退款。",
            "退换": "亲，很抱歉给您带来不便。请提供订单号和商品照片，我会生成退换货审核草稿，待人工客服确认后处理。",
            "优惠": "亲，我可以帮您查询当前活动配置；具体优惠以店铺后台和客服确认结果为准。",
        }
        for keyword, reply in reply_map.items():  # 遍历关键词，返回第一个匹配的回复
            if keyword in content:
                return reply
        return "亲，感谢您的咨询。我会先生成待人工确认的回复草稿，避免在未核实数据源前承诺业务状态。"  # 完全不匹配时的通用兜底回复

    def _calculate_confidence(
        self, reply: str, emotion: EmotionLevel
    ) -> float:  # 综合回复质量和情绪计算置信度
    ...
    def _determine_send_decision(
        self,
        confidence: float,  # 基于置信度和情绪决定三个发送级别
        emotion: EmotionLevel,
    ) -> SendDecision:
        if emotion == EmotionLevel.HIGH_RISK:  # 高风险消息无论置信度如何都必须人工审核
            return SendDecision.MANUAL_REVIEW
        if confidence >= 0.90 and _customer_auto_send_enabled():  # 置信度≥90%且显式启用才可自动发送
    ...
    def _add_to_batch_review(
        self, draft: ReplyDraft, company_id: int
    ):  # 将回复草稿加入批量审核队列
    ...
    async def _handle_emotion_escalation(
        self,
        message: CustomerMessage,  # 高风险消息触发升级告警
        emotion: EmotionLevel,
        company_id: int,
    ):
    ...
    async def execute_send(
    ...
    ) -> dict:  # 执行发送操作，仅显式启用自动外发且AUTO_SEND决策才会实际发送
        if draft.send_decision != SendDecision.AUTO_SEND or not _customer_auto_send_enabled():
            return {
                "status": "pending_review",
                "conv_id": draft.conv_id,
        ...
        try:
            ...
            result = cs_integration.send_reply(
                draft.conv_id, draft.content, auto_send=True
            )  # 调用企业系统接口发送
            return result
