...
RAGAS_AVAILABLE = find_spec("ragas") is not None  # ragas 是可选依赖，实际评估时再延迟导入
if not RAGAS_AVAILABLE:  # ragas 未安装
...
class RAGEvaluator:  # RAG 检索与生成质量评估器
    ...
    def hit_rate_at_k(
        self, query_results: list[list[str]], relevant_docs: list[list[str]], k: int = 5
    ) -> float:  # Hit Rate@k：top-k 中是否包含相关文档
        if not query_results or not relevant_docs:  # 空数据检查
            return 0.0
        hits = 0  # 命中计数
        for results, relevant in zip(query_results, relevant_docs, strict=False):  # 逐查询比对
    ...
    def mrr(
        self, query_results: list[list[str]], relevant_docs: list[list[str]]
    ) -> float:  # MRR：平均倒数排名
        if not query_results or not relevant_docs:  # 空数据检查
            return 0.0
        reciprocal_ranks = []  # 每个查询的倒数排名
        for results, relevant in zip(query_results, relevant_docs, strict=False):  # 逐查询计算
    ...
    def ndcg_at_k(
        self, query_results: list[list[str]], relevance_scores: list[dict[str, int]], k: int = 10
    ) -> float:  # NDCG@k：归一化折损累计增益
        if not query_results or not relevance_scores:  # 空数据检查
            return 0.0
        ndcg_values = []  # 每个查询的 NDCG
        for results, scores in zip(query_results, relevance_scores, strict=False):  # 逐查询计算
            dcg = 0.0  # DCG：折损累计增益
    ...
    def evaluate_faithfulness(
        self, questions: list[str], contexts: list[str], answers: list[str]
    ) -> dict:  # 忠实度评估：回答是否基于上下文
        ...
        try:  # ragas 评估可能失败
            ...
            scores = []  # 收集分数
            for q, ctx, ans in zip(questions, contexts, answers, strict=False):  # 逐样本评估
                sample = SingleTurnSample(
                    user_input=q, retrieved_contexts=[ctx], response=ans
                )  # 构建样本
    ...
    def evaluate_answer_relevancy(
        ...
        try:  # ragas 评估可能失败
            ...
            scores = []  # 收集分数
            for q, ans in zip(questions, answers, strict=False):  # 逐样本评估
    ...
    def load_dataset(self) -> list[dict]:  # 加载评估数据集
        ...
        if not path:  # 未指定时使用默认路径
            path = (
                Path(__file__).parent.parent.parent / "tests" / "rag_eval_dataset.json"
            )  # 相对于 rag_evaluator.py 向上三级到项目根目录
        try:  # 文件可能存在
            with open(path, encoding="utf-8") as f:  # UTF-8 编码
    ...
    def run_full_evaluation(self) -> dict:  # 运行完整评估
        ...
        [d["query"] for d in dataset]  # 提取所有查询
        ...
        report = {  # 构建评估报告
            ...
            "retrieval_metrics": {  # 检索指标
                ...
                "hit_rate@10": self.hit_rate_at_k(
                    query_results, relevant_docs, k=10
                ),  # Hit Rate@10
        ...
        if RAGAS_AVAILABLE and all(
            d.get("answer") and d.get("context") for d in dataset
        ):  # 有生成结果时才评估生成指标
...
@lru_cache(maxsize=1)
def get_rag_evaluator(eval_dataset_path: str | None = None) -> RAGEvaluator:
    """获取进程级 RAG 评估器实例，供 API 配置页和测试复用。"""
    return RAGEvaluator(eval_dataset_path=eval_dataset_path)
