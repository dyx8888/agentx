...
from app.auth import get_current_active_user  # A2A 操作需要认证：Agent 发现和任务委派都应受身份验证保护
from app.communication.a2a_adapter import get_a2a_adapter  # A2A 适配器单例：封装 Agent 发现和任务委派的底层逻辑
...
class TaskRequest(BaseModel):  # A2A 任务委派请求
    ...
    target_agent_name: str  # 目标 Agent 名称（而非 ID），便于人类可读的发现和路由
    task: str  # 自然语言任务描述，由目标 Agent 自行解析和执行
...
@router.get("/.well-known/agent.json")  # A2A 标准发现端点，其他 Agent 可通过此端点获取当前服务的 Agent Card
async def get_agent_card(current_user: User = Depends(get_current_active_user)):
    ...
    try:
        ...
        agent_card = {
            ...
            "company_id": current_user.company_id if current_user else 1,  # 安全获取：未认证用户默认公司 ID 为 1
            "status": "active",  # 固定为 active，表示服务在线可用
            "registered_at": datetime.utcnow().isoformat() + "Z",  # UTC 时间后缀 Z，符合 ISO 8601 / A2A 协议规范
        ...
        logging.error(f"Error generating agent card: {e}")  # 使用标准库 logging 而非 app logger
        raise HTTPException(status_code=500, detail=f"Failed to generate agent card: {str(e)}")
...
async def discover_agents(current_user: User = Depends(get_current_active_user)):
    ...
    try:
        ...
        agent_cards = adapter.discover_agents(current_user.company_id if current_user else 1)  # 按公司发现 Agent
        ...
        return {
            ...
            "agents": agent_cards,
            "protocol": "a2a",  # 协议标识
            "timestamp": datetime.utcnow().isoformat() + "Z"  # 返回时间戳便于缓存策略
        }
...
async def a2a_delegate_task(
    ...
    try:
        ...
        return TaskResponse(  # 即使失败也返回 200，通过 success 字段区分：保持 HTTP 连接不被中断
            ...
            timestamp=datetime.utcnow().isoformat() + "Z",  # 记录失败时间
            error=str(e)
        )
