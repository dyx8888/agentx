...
class TaskConfirmationRequest(BaseModel):
    step_id: int | None = None  # 需要确认的步骤 ID（status 模式下可为空）
    action: str | None = None  # 确认动作：approve/reject/modify（status 模式下可为空）
    modified_content: str | None = None  # modify 时的修改内容
    status: str | None = None  # 直接状态更新模式：提供此字段时跳过步骤确认逻辑
    message: str | None = None  # 状态更新附带的说明消息
...
async def create_task(
    request: CreateTaskRequest, current_user: User = Depends(get_current_active_user)
):
    """
...
async def get_pending_tasks(current_user: User = Depends(get_current_active_user)):
    """
    ...
    try:
        ...
        # 在 SQL 查询中带 WHERE company_id 条件，避免内存过滤导致的跨租户风险
        with get_engine().connect() as conn:
            result = conn.execute(
                text("""
                    SELECT id, company_id, source_agent_id, target_agent_name,
                           task_description, created_at
                    FROM tasks
                    WHERE status = 'pending' AND company_id = :company_id
...
async def cancel_task(task_id: int, current_user: User = Depends(get_current_active_user)):
    """
...
@router.get("/{task_id:int}/steps", response_model=TaskStepsResponse)
async def get_task_steps(task_id: int, current_user: User = Depends(get_current_active_user)):
    """
...
@router.post("/{task_id:int}/confirm")
async def confirm_task_step(
    ...
    try:
        ...
        # 第二步半：直接状态更新模式（提供 status 字段时跳过步骤确认逻辑）
        if request.status is not None:
            db.update_task_status(task_id, request.status)
            try:
                from app.ws import broadcast_task_status_update

                await broadcast_task_status_update(task_id, request.status, request.message)
            ...
            return {"status": "updated", "task_id": task_id, "new_status": request.status}
        ...
        if request.action == "approve":
            ...
            if not request.modified_content:
                raise HTTPException(
                    status_code=400, detail="modified_content is required for modify action"
                )
            target_step["status"] = "completed"
            ...
            raise HTTPException(
                status_code=400, detail="Invalid action. Must be approve, reject, or modify"
            )
        ...
        logger.exception("confirm_task_step_failed")
        raise HTTPException(status_code=500, detail="内部服务器错误")
