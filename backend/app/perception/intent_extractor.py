...
class IntentType(StrEnum):  # 用 StrEnum 而非 Enum 是为了 intent_type.value 直接返回字符串，无需额外转换
...
class Intent:
    ...
    entities: dict = field(default_factory=dict)  # 用 field 而非 ={} 避免多个实列共享同一个 dict 对象
    sub_intents: list["Intent"] = field(default_factory=list)  # 前向引用用字符串类型，因为 Intent 类尚未定义完毕
    is_malicious: bool = False  # 默认不视为恶意，假阳性代价远大于假阴性
    raw_response: str = ""  # 保留原始 LLM 响应用于调试和审计追溯

    def to_dict(self) -> dict:  # 序列化为字典用于日志和 API 响应，不暴露 raw_response 避免泄露内部信息
        return {
...
class IntentExtractor:
    ...
    def __init__(self, model_gateway=None):  # 默认参数为 None 支持延迟初始化，避免在模块加载时就创建 gateaway
        self._model_gateway = model_gateway

    def _get_llm(self):  # 懒加载 gateaway 的 LLM 实例，首次调用时才真正初始化
    ...
    def extract(self, text: str) -> Intent:
        ...
        try:
            llm = self._get_llm()
            from langchain_core.messages import HumanMessage, SystemMessage  # 延迟导入，不强制项目必须安装 langchain

            messages = [
                SystemMessage(content=INTENT_SYSTEM_PROMPT),  # SystemMessage 让 LLM 以意图分析专家身份思考
                HumanMessage(content=text),
            ]
            response = llm.invoke(messages)
            raw = response.content.strip() if hasattr(response, 'content') else str(response).strip()  # 兼容多个 LLM 库的响应格式

            return self._parse_response(raw, text)
        except Exception as e:
            logger.warning("intent_extraction_failed", error=str(e))  # warning 而非 error，因为降级到 GENERAL 是可接受的
    ...
    def _parse_response(self, raw: str, original_text: str) -> Intent:
        ...
        json_match = re.search(r'\{[\s\S]*\}', raw)  # 用 [\s\S] 而非 . 是因为 . 不匹配换行，而 LLM 返回的 JSON 可能跨多行
        ...
        try:
            ...
            logger.warning("intent_json_parse_failed", raw=raw[:200])  # 只记录前 200 字符避免敏感数据日志泄露
            return Intent(
        ...
        confidence = max(0.0, min(1.0, confidence))  # clamp 到 [0,1] 区间，因为 LLM 可能返回超范围浮点数
        ...
        is_malicious = bool(data.get("is_malicious", False))  # 显式 bool 转换防止 LLM 返回字符串 "false"
