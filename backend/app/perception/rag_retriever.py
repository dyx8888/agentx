...
class RagResult:  # 独立的 dataclass 而非嵌套在 Retriever 中，便于其他模块直接引用类型
    ...
    knowledge_results: list[dict] = field(default_factory=list)  # 原始知识库结果，保留用于后续精确引用
    experience_results: list[dict] = field(default_factory=list)  # 经验库结果，与小知识分离存储便于不同展示策略
...
class RagRetriever:
    ...
    def retrieve(
        ...
        try:
            from app.rag.agentic_rag import get_agentic_rag  # 延迟导入，RAG 初始化开销大且可能有循环依赖

            rag = get_agentic_rag(company_id)  # 按公司 ID 获取专属 RAG 实例，实现租户数据隔离
            rag_context = rag.retrieve(query=query, agent_name=agent_name, top_k=top_k)  # 获取拼接好的上下文文本

            structured = rag.retrieve_structured(query=query, agent_name=agent_name, top_k=top_k)  # 结构化结果用于生成引用
            ...
            refs = [  # 构建引用列表，每个引用截取前 200 字符避免过大
                ...
                for r in structured.get("knowledge_results", [])  # 只从知识结果中提取引用，经验结果通常不展示来源
            ]
            ...
            return RagResult(
                context=rag_context,
                references=refs,
                knowledge_results=structured.get("knowledge_results", []),  # 保留原始数据方便下游做更多处理
                experience_results=structured.get("experience_results", []),
                query=query,
                intent_type=intent_type,
            )
        except Exception as e:
            logger.warning("rag_retrieve_failed", error=str(e), agent=agent_name, company_id=company_id)  # 包含上下文信息便于排查
            return RagResult(query=query, intent_type=intent_type)  # 降级返回空结果，不阻断管线

    def augment_message(self, message: str, rag_result: RagResult) -> str:  # 独立方法便于单独测试注入逻辑
        """将 RAG 检索上下文注入到用户消息中"""
