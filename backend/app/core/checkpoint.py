...
import random  # get_next_version 中生成随机后缀，避免分布式下版本号碰撞
from collections import defaultdict  # 内存降级存储的嵌套字典结构，自动创建缺失键减少判断代码
from contextlib import ExitStack  # 预留上下文管理器集合，用于统一管理需要清理的资源（如连接池、文件句柄）
from typing import Any, AsyncIterator, Iterator, Optional, Sequence  # 类型标注提供自文档化能力，减少运行时的类型错误
...
try:
    ...
    get_checkpoint_id = lambda c: c.get("configurable", {}).get("checkpoint_id")  # lambda 提取 checkpoint_id，兼容旧版 config 结构
    get_checkpoint_metadata = lambda c, m: m  # 旧版中 metadata 直接传入，无需额外处理
    WRITES_IDX_MAP = {}

try:
    from langgraph.serde.jsonplus import JsonPlusSerializer  # JsonPlus 支持更丰富的类型序列化（如 datetime、set）
except ImportError:
...
class RedisSaver(BaseCheckpointSaver):
    ...
    def __init__(self, redis_url: Optional[str] = None, ttl: int = DEFAULT_CHECKPOINT_TTL):
        serde = JsonPlusSerializer() if JsonPlusSerializer else None  # 序列化器优先使用 JsonPlus，支持更多类型
        super().__init__(serde=serde)  # 调用父类初始化，传入序列化器以注册到 LangGraph 框架
        self._redis_url = redis_url or os.getenv("REDIS_URL", "redis://localhost:6379/0")  # 优先参数 > 环境变量 > 默认值，保证灵活性
        self._redis_client: Optional[redis.Redis] = None  # 延迟初始化：连接在 _connect() 中建立，避免构造时阻塞
        self._connection_pool: Optional[ConnectionPool] = None  # 连接池复用连接，避免频繁 TCP 握手
        self._connected: bool = False  # 显式标记连接状态，用 bool 而非 try/except 判断，性能更好且语义清晰
        self._ttl = ttl  # 实例级 TTL 允许不同场景使用不同过期时间

        # 内存降级存储
        self._storage: dict = defaultdict(lambda: defaultdict(dict))  # 三层嵌套：thread_id -> checkpoint_ns -> checkpoint_id -> data
        self._writes: dict = defaultdict(dict)  # pending writes 缓存：(thread_id, checkpoint_ns, checkpoint_id) -> writes
    ...
    def _connect(self) -> bool:  # 返回 bool 让调用方可判断连接是否成功，但不建议依赖返回值（连接失败内部已降级）
        if redis is None:  # redis-py 未安装时直接跳过，不抛异常
            logger.warning("checkpoint_redis_not_installed")
            return False
        try:
            self._connection_pool = ConnectionPool.from_url(self._redis_url)  # 连接池可复用 TCP 连接，减少 Redis 连接开销
    ...
    @property
    def is_available(self) -> bool:  # property 提供简洁的只读访问，外部不需要知道 _connected 的内部实现细节
        return self._connected and self._redis_client is not None  # 双重检查：连接标记 + 客户端实例，避免竞态导致 NPE

    # ── Redis Key 构建 ──────────────────────────────────────────

    @staticmethod
    def _key_checkpoint(thread_id: str, checkpoint_ns: str, checkpoint_id: str) -> str:  # staticmethod 不需要实例状态，key 构建逻辑与 Redis 连接状态无关
        return f"ckpt:{thread_id}:{checkpoint_ns}:{checkpoint_id}"  # 用冒号分隔形成层级结构，Redis 可视化工具可按层级浏览

    @staticmethod
    def _key_latest(thread_id: str, checkpoint_ns: str) -> str:  # latest 指针独立存储，避免每次都遍历所有 checkpoint 找最新的
        return f"ckpt:{thread_id}:{checkpoint_ns}:latest"

    @staticmethod
    def _key_blob(thread_id: str, checkpoint_ns: str, channel: str, version: str) -> str:  # blob 按 channel+version 隔离，支持增量更新
        return f"ckpt:blob:{thread_id}:{checkpoint_ns}:{channel}:{version}"

    @staticmethod
    def _key_writes(thread_id: str, checkpoint_ns: str, checkpoint_id: str) -> str:  # writes 与 checkpoint 绑定，用于恢复时重放未完成的写入
    ...
    def _serialize(self, obj: Any) -> bytes:  # 实例方法：需要访问 self.serde 属性
        if self.serde:  # JsonPlusSerializer 优先，支持 datetime/set 等原生 JSON 不支持的类型
            return self.serde.dumps_typed(obj)[1]  # dumps_typed 返回 (type_tag, serialized_bytes)，取第二部分
        return json.dumps(obj, default=str).encode()  # 降级到 json.dumps，default=str 确保任何对象都能被序列化而不抛异常

    def _deserialize(self, raw: bytes) -> Any:
        if self.serde:
            return self.serde.loads_typed(("json", raw))  # loads_typed 需要 type_tag 参数，这里固定为 "json"
        return json.loads(raw)  # 纯 JSON 反序列化，不需要类型信息

    # ── 核心 API ────────────────────────────────────────────────

    def get_tuple(self, config: dict) -> Optional[CheckpointTuple]:  # LangGraph 框架要求的接口方法，返回 Optional 表示可能不存在
        thread_id = config["configurable"]["thread_id"]  # configurable 是 LangGraph 的嵌套配置字典，外部 key 固定
        checkpoint_ns = config["configurable"].get("checkpoint_ns", "")  # checkpoint_ns 可选，空字符串作为默认命名空间

        checkpoint_id = get_checkpoint_id(config) if get_checkpoint_id else None  # 兼容旧版：get_checkpoint_id 可能为 None
    ...
    def _get_specific(
        ...
        data = self._fetch_checkpoint(thread_id, checkpoint_ns, checkpoint_id)  # 先查 Redis 再降级内存，统一入口
        if not data:
            return None
        return self._build_tuple(thread_id, checkpoint_ns, checkpoint_id, data, config)  # 组装 CheckpointTuple，加载 blobs 和 pending writes
    ...
    def _get_latest(
        ...
        if self.is_available:  # 先检查可用性，避免不必要的 try/except 开销
            try:
                latest_id = self._redis_client.get(
                    self._key_latest(thread_id, checkpoint_ns)
                )
                if latest_id:
        ...
        # 降级到内存
        if not latest_id and self._storage[thread_id][checkpoint_ns]:  # 双层检查：latest_id 为空 + 内存有数据
            checkpoints = self._storage[thread_id][checkpoint_ns]
            latest_id = max(checkpoints.keys())  # 用 checkpoint_id 的字符串排序取最大，因为 ID 格式为单调递增的版本号
    ...
    def _fetch_checkpoint(
        ...
        entry = self._storage[thread_id][checkpoint_ns].get(checkpoint_id)  # 内存降级，defaultdict 确保不抛 KeyError
        return entry
    ...
    def _build_tuple(
        ...
        checkpoint_raw = data.get("checkpoint", {})  # 从存储数据中提取 checkpoint body，.get 防御性取值以防数据结构不完整
        ...
        checkpoint = {**checkpoint_raw, "channel_values": channel_values}  # 合并 blob 数据到 checkpoint，保持 LangGraph 的标准结构

        # 加载 pending writes
        writes_map = self._fetch_writes(thread_id, checkpoint_ns, checkpoint_id)  # pending writes 是未完成写入，恢复时需重放
        pending_writes = list(writes_map.values())  # 转换为列表，LangGraph 的 CheckpointTuple 期望 list 格式
    ...
    def _fetch_writes(
        ...
        return self._writes.get((thread_id, checkpoint_ns, checkpoint_id), {})  # 内存降级返回空 dict，不抛异常
        ...
        for channel, version in versions.items():
            ...
            if self.is_available:
                try:
                    ...
                    if raw:
                        vv = raw
                        if vv != b"empty":  # 哨兵值 "empty" 表示该 channel 在本版本中为空，避免 None 混淆
    ...
    def put(
        ...
        checkpoint_id = checkpoint["id"]  # 从 checkpoint 对象中提取 id，而非从 config 中，保证一致性

        c = checkpoint.copy()  # 浅拷贝避免修改原始对象，深拷贝会浪费内存
        channel_values = c.pop("channel_values", {})  # channel_values 分离存储为 blob，减小 checkpoint 主记录体积
        ...
        for channel, version in new_versions.items():
            ...
            if channel in channel_values:  # 有值则序列化存储
                if self.is_available:
                    ...
                    except Exception:  # Redis 失败不阻塞，内存降级继续
                        pass
                self._blobs[(thread_id, checkpoint_ns, channel, version)] = channel_values[channel]  # 内存始终存储，作为 Redis 的双重保障
        ...
        # 存储 checkpoint
        parent_id = config["configurable"].get("checkpoint_id")  # 当前 config 中的 checkpoint_id 即为父节点
        data = {
            "checkpoint": c,
            "metadata": get_checkpoint_metadata(config, metadata) if get_checkpoint_metadata else metadata,
        ...
        if self.is_available:
            try:
                ...
                pipe = self._redis_client.pipeline()  # 使用 pipeline 批量执行，保证原子性：checkpoint 和 latest 指针同时更新
        ...
        # 内存降级
        self._storage[thread_id][checkpoint_ns][checkpoint_id] = data  # 内存始终存储，不依赖 Redis 成功
    ...
    def put_writes(
        ...
        outer_key = (thread_id, checkpoint_ns, checkpoint_id)  # 用 tuple 作为复合键，比字符串拼接更高效且避免分隔符冲突
        existing = self._writes.get(outer_key, {})  # 获取已有 writes，追加而非覆盖

        for idx, (channel, value) in enumerate(writes):
            inner_key = (task_id, WRITES_IDX_MAP.get(channel, idx) if WRITES_IDX_MAP else idx)  # WRITES_IDX_MAP 提供 channel 的固定索引映射
        ...
        if self.is_available:
            try:
                ...
                self._redis_client.setex(
                    writes_key, self._ttl, self._serialize(existing)
                )
            except Exception:
                pass
    ...
    def delete_thread(self, thread_id: str) -> None:
        if self.is_available:
            try:
                pattern = f"ckpt:{thread_id}:*"  # 用通配符模式匹配该线程的所有 key，批量删除而非逐个指定
                for key in self._redis_client.scan_iter(match=pattern):  # scan_iter 避免 KEYS 阻塞 Redis，适合生产环境
                    self._redis_client.delete(key)
                pattern = f"ckpt:blob:{thread_id}:*"  # blob 和 writes 需要单独删除，因为 key 前缀不同
    ...
    def get_next_version(self, current: Optional[str], channel: None) -> str:  # channel 参数为 None 是 LangGraph 的接口约定
        if current is None:
        ...
        next_v = current_v + 1
        next_h = random.random()  # 随机小数部分防止分布式下多节点同时写入产生版本号冲突
        return f"{next_v:032}.{next_h:016}"  # 零填充保证字典序排序正确，版本号即为排序键

    # ── 异步方法 ─────────────────────────────────────────────────

    async def aget_tuple(self, config: dict) -> Optional[CheckpointTuple]:  # async 方法直接委托同步方法，因为 Redis 操作本身是 I/O 阻塞的，这里提供兼容接口
    ...
    async def alist(
        ...
        *,
        ...
        # 简化版：返回内存中的结果
        for thread_id in (self._storage if not config else [config["configurable"]["thread_id"]]):  # 无 config 时遍历所有线程
            for checkpoint_ns in self._storage.get(thread_id, {}):
                for checkpoint_id, data in sorted(
                    ...
                    key=lambda x: x[0], reverse=True,  # 降序排列，最新的在前
                ):
...
# 全局单例
_redis_saver: Optional[RedisSaver] = None  # 模块级变量，延迟初始化
