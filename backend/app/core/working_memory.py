...
from dataclasses import (  # dataclass 自动生成 __init__/__repr__/__eq__，减少样板代码；field 提供 default_factory 避免可变默认值陷阱
    dataclass,
    field,
)
...
class WorkingMemory:
    ...
    goal: str = (
        ""  # 默认空字符串而非 None：在 Prompt 模板中空字符串比 None 更安全，不会出现 "None" 文字
    )
    """当前任务的总体目标描述"""
    ...
    temporary_variables: dict = field(
        default_factory=dict
    )  # field(default_factory=dict) 避免所有实例共享同一个 dict 对象
    """任务执行中的临时变量和中间结果
    ...
    )  # 用 list 而非 set：动作顺序很重要，需要保留执行顺序
    """尚未完成的待处理动作列表"""

    @property
    def is_complete(
        self,
    ) -> bool:  # property 而非普通字段：状态由 pending_actions 派生，避免数据冗余和不一致
        """任务是否已完成（无待处理动作）"""
    ...
    def step_count(
        self,
    ) -> int:  # 派生属性：从 temporary_variables 中的 step_results 计算，而非单独维护一个计数器
        """已完成的步骤数"""
        results = self.temporary_variables.get("step_results", [])
        return (
            len(results) if isinstance(results, list) else 0
        )  # isinstance 防御性检查：step_results 可能被意外赋值为非 list 类型
    ...
    def from_dict(
        cls, data: dict
    ) -> "WorkingMemory":  # classmethod 而非 staticmethod：需要访问 cls 来创建实例，支持子类继承
        """从 dict 反序列化"""
        return cls(
