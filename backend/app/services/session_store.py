...
from langchain_core.messages import (  # LangChain消息类型，用于构建对话上下文
    AIMessage,
    BaseMessage,
    HumanMessage,
    SystemMessage,
...
SESSION_MAX_TOKENS = int(
    os.getenv("SESSION_MAX_TOKENS", "8000")
)  # 8000 tokens，适配大多数模型的上下文窗口
SESSION_SUMMARY_TTL = int(
    os.getenv("SESSION_SUMMARY_TTL_SECONDS", "604800")
)  # 7天摘要保留，比会话历史更长
...
class SessionStore:  # 会话存储：支持Redis和内存两种后端，Redis不可用时自动降级为内存存储
    ...
    def _init_redis(
        ...
        try:  # 外层try包裹，Redis不可用不应阻止应用启动
            ...
            )  # from_url仅构造客户端对象，连接在首次命令时惰性建立
            # 注意：ping()是异步的，无法在同步构造函数中调用；连接验证延迟到init()中执行
            # _redis_available保持False，直到init()中await ping()成功后才置为True
        except Exception as e:  # 宽泛捕获，Redis不可用不是致命错误
            logger.warning(
                "session_store_redis_unavailable", error=str(e)
            )  # warning级别，因为系统仍可运行

    async def init(
        self,
    ):  # 异步初始化：验证Redis连接，需在应用启动时（如FastAPI lifespan）await调用
        """异步初始化Redis连接：ping成功后标记可用，失败则保持内存降级模式

        必须在事件循环中调用，建议在 main.py 的 lifespan 启动阶段执行：
            await get_session_store().init()
        """
        if not self._redis:  # 客户端未创建（_init_redis失败），无需ping
            return
        try:
            await self._redis.ping()  # 异步ping验证连接，不阻塞事件循环
            self._redis_available = True  # 只有ping成功才标记可用
            logger.info("session_store_redis_connected", url=REDIS_URL)
        except Exception as e:  # 宽泛捕获，Redis不可用不是致命错误
            self._redis_available = False  # 保持内存降级模式
            logger.warning("session_store_redis_unavailable", error=str(e))

    async def close(self):  # 异步关闭Redis连接，需在应用关闭时（如FastAPI lifespan）await调用
        """关闭异步Redis连接，释放连接池资源

        建议在 main.py 的 lifespan 关闭阶段执行：
            await get_session_store().close()
        """
        if self._redis is not None:
            try:
                await self._redis.aclose()  # redis.asyncio使用aclose()关闭连接及连接池
            except Exception as e:
                logger.warning("session_store_redis_close_error", error=str(e))
            finally:
                self._redis = None  # 置空避免重复关闭
                self._redis_available = False  # 标记不可用，后续操作降级为内存

    def _estimate_tokens(
        self, messages: list[BaseMessage]
    ) -> int:  # 估算token数，避免调用tokenizer的性能开销
    ...
    async def _summarize_truncated(
        self, truncated: list[BaseMessage]
    ) -> str | None:  # 异步方法，使用LLM生成摘要，避免阻塞主线程
...
只输出摘要文本，不要加任何前缀或格式标记。"""  # 明确指令防止LLM输出多余内容
        ...
        try:  # 摘要生成失败不应影响正常的消息存储流程
            from langchain_core.messages import (
                HumanMessage as HMsg,  # 使用别名避免与导入的HumanMessage冲突
            )

            from app.services.model_gateway import (
                get_global_model_gateway,  # 延迟导入，避免循环依赖
            )
    ...
    def _merge_summary(
        self, summary: str | None, messages: list[BaseMessage]
    ) -> list[BaseMessage]:  # 将摘要作为SystemMessage注入到消息列表开头
    ...
    async def get_history(
        self, session_id: str
    ) -> list[BaseMessage]:  # 异步方法：Redis get需await，避免阻塞事件循环
        ...
        try:  # 外层try包裹，确保存储层异常不会导致应用崩溃
            ...
            if self._redis_available:  # Redis可用时优先使用Redis
                raw = await self._redis.get(f"session:{session_id}")  # 异步get，避免阻塞事件循环
                if raw:  # 仅在有数据时反序列化
                    messages = self._deserialize_messages(
                        raw.decode("utf-8")
                    )  # Redis返回bytes，需要decode
                raw_summary = await self._redis.get(
    ...
    async def append_exchange(
        ...
        try:  # 外层try确保存储异常不影响主流程
            history = await self.get_history(session_id)  # 异步获取现有历史，避免阻塞事件循环
            # 去掉可能的摘要前缀消息，做纯 fact 历史
            pure_history = [
                m
                for m in history
                if not (  # 过滤掉摘要SystemMessage，因为新一轮会重新生成
                    isinstance(m, SystemMessage) and m.content.startswith("[历史会话摘要]")
                )
            ]
            ...
            if len(pure_history) > SESSION_MAX_MESSAGES:  # 双重保护：token截断后消息数仍可能超限
                overflow = pure_history[
                    : len(pure_history) - SESSION_MAX_MESSAGES
                ]  # 取出超出的部分
            ...
            if self._redis_available:  # Redis可用时写入Redis
                await self._redis.setex(
                    f"session:{session_id}", SESSION_TTL, serialized
                )  # 异步setex，避免阻塞事件循环
                if summary:  # 有摘要时单独存储
                    await self._redis.setex(
                        f"session:{session_id}:summary",
    ...
    async def clear_session(
        self, session_id: str
    ):  # 异步方法：Redis delete需await，避免阻塞事件循环
        try:
            if self._redis_available:  # Redis可用时删除Redis中的key
                await self._redis.delete(f"session:{session_id}")  # 异步delete
                await self._redis.delete(f"session:{session_id}:summary")  # 异步delete
    ...
    async def set_cache(
        self, key: str, data: str, ttl: int = 86400
    ):  # 异步方法：Redis setex需await，避免阻塞事件循环
        try:
            if self._redis_available:  # Redis可用时使用Redis
                await self._redis.setex(key, ttl, data)  # 异步setex
