...
class ModelCapability:  # 描述每个模型的能力和成本，用于路由决策
    ...
    suitable_for: list[str] = field(default_factory=lambda: ["standard"])  # 适合的任务复杂度，使用field避免可变默认值
...
class EnterpriseKeyManager:  # 企业自有API Key管理器，支持多租户Key隔离
    ...
    def __init__(self):  # 初始化空字典，Key通过set_keys或_load_from_db填充
        self._company_keys: dict[int, dict[str, str]] = {}  # 嵌套字典：company_id → {provider: api_key}

    def set_keys(self, company_id: int, provider: str, api_key: str):  # 手动设置企业Key，适用于管理后台操作
    ...
    def get_key(self, company_id: int, provider: str) -> str | None:  # 获取企业Key，缓存未命中时从数据库加载
    ...
    def _load_from_db(self, company_id: int):  # 从数据库加载企业Key，使用延迟导入避免循环依赖
        ...
        try:  # 数据库加载失败不应影响主流程
            ...
            creds = json.loads(credentials) if isinstance(credentials, str) else credentials  # 兼容字符串和字典两种格式
...
class TokenQuotaManager:  # Token配额管理器，按企业ID隔离，支持日配额和月配额
    ...
    def set_quota(self, company_id: int, daily_limit: int = 0, monthly_limit: int = 0):  # 设置企业配额，0表示不限制
    ...
    def check_quota(self, company_id: int, tokens: int) -> tuple[bool, str]:  # 检查配额是否足够，返回(是否允许, 原因)
        """检查配额是否足够"""
        ...
        if quota["daily_limit"] > 0 and quota["daily_used"] + tokens > quota["daily_limit"]:  # 日配额检查，limit>0才限制
            return False, f"Daily token quota exceeded ({quota['daily_limit']})"
        if quota["monthly_limit"] > 0 and quota["monthly_used"] + tokens > quota["monthly_limit"]:  # 月配额检查
...
class ModelRouter:  # 模型智能路由器，全部使用classmethod，因为路由规则是类级别配置
    """模型智能路由器 - 按任务复杂度 + Agent 类型自动选模型"""

    ROUTING_RULES = {  # 类级别路由规则表，Agent类型 → 复杂度 → 模型key
        "品牌商务": {  # 品牌商务Agent：简单/标准用deepseek_chat，复杂用deepseek_reasoner
            TaskComplexity.SIMPLE: "deepseek_chat",
            TaskComplexity.STANDARD: "deepseek_chat",
            TaskComplexity.COMPLEX: "deepseek_reasoner",  # 复杂任务需要推理能力
        },
        "内容运营": {  # 内容运营Agent：复杂任务用GPT-4，因为内容生成质量要求高
            TaskComplexity.SIMPLE: "deepseek_chat",
            TaskComplexity.STANDARD: "deepseek_chat",
            TaskComplexity.COMPLEX: "openai_gpt4",  # 内容运营的复杂任务对质量要求最高
        },
        "数据分析": {  # 数据分析Agent：复杂任务用deepseek_reasoner
            TaskComplexity.SIMPLE: "deepseek_chat",
            TaskComplexity.STANDARD: "deepseek_chat",
            TaskComplexity.COMPLEX: "deepseek_reasoner",
        },
        "客服专员": {  # 客服Agent：简单任务用低成本volcano_lite，节省成本
            TaskComplexity.SIMPLE: "volcano_lite",  # 简单回复用最便宜的模型
            TaskComplexity.STANDARD: "deepseek_chat",
            TaskComplexity.COMPLEX: "deepseek_chat",
        },
        "视觉设计": {  # 视觉设计Agent：标准/复杂用GPT-4，因为视觉理解需要多模态能力
            TaskComplexity.SIMPLE: "deepseek_chat",
            TaskComplexity.STANDARD: "openai_gpt4",
            TaskComplexity.COMPLEX: "openai_gpt4",
        },
    ...
    def get_model(cls, agent_name: str = None, complexity: TaskComplexity = None) -> str:  # 根据Agent名称和复杂度返回模型key
        agent_rules = cls.ROUTING_RULES.get(agent_name, cls.ROUTING_RULES["default"])  # 未知Agent使用默认规则
        return agent_rules.get(complexity, "deepseek_chat")  # 未知复杂度默认deepseek_chat

    @classmethod  # classmethod，估算逻辑基于类级别关键词
    def estimate_complexity(cls, task_description: str) -> TaskComplexity:  # 基于关键词的简单复杂度估算
        """根据任务描述估算复杂度"""
...
class TokenBudgetManager:
    ...
    def __init__(self, window_size: int = DEFAULT_TOKEN_WINDOW,
    ...
    def estimate_input_tokens(self, system_prompt: str = "",
                               user_prompt: str = "",
                               history_messages: list = None,
                               rag_context: str = "",
                               tool_schemas: list = None) -> int:
    ...
    def check_budget(self, input_tokens: int,
                     reasoning_tokens: int = 0) -> tuple[bool, dict]:
        """检查Token预算是否足够
        ...
        budget_info = {
            ...
            "usage_ratio": round(total_estimated / effective_window, 3) if effective_window > 0 else 1.0,
            "is_within_budget": is_within,
        }
        self._budget_history.append(budget_info)
        return is_within, budget_info

    def get_degrade_strategy(self, input_tokens: int,
...
class RetryWithIdempotency:
    ...
    @staticmethod
    def generate_idempotent_key(company_id: str, agent_key: str,
                                 message: str, window_minutes: int = 60) -> str:
...
class FailoverChatModel:  # 故障转移聊天模型：主模型失败时自动切换到fallback模型，确保高可用
    ...
    def __init__(self, gateway: 'ModelGateway', model_key: str,
    ...
    def _init_models(self):  # 初始化模型实例列表，主模型+fallback按顺序排列
        ...
        self.models = [self.gateway._create_model_instance(self.model_key, self.company_api_key)]  # 主模型放在第一位
        ...
        for i, model in enumerate(self.models):  # 按顺序尝试模型，先主后fallback
            try:
                ...
                if hasattr(model, 'ainvoke'):  # 优先使用异步调用
                    resp = await model.ainvoke(messages, **kwargs)
                else:  # 降级为同步调用
    ...
    def bind_tools(self, tools: list):  # 绑定工具，仅主模型支持
        ...
        if hasattr(self.models[0], 'bind_tools'):  # 检查主模型是否支持bind_tools
            self.models[0] = self.models[0].bind_tools(tools)  # 替换主模型为绑定工具后的版本
            return self
        raise AttributeError("Primary model doesn't support bind_tools")

    def _record_usage(self, model_name: str, response, messages: list[BaseMessage]):  # 记录每次调用的token用量和成本
        try:  # 成本追踪失败不应影响主流程
            input_tokens = getattr(getattr(response, 'usage', None), 'prompt_tokens', 0)  # 安全获取输入token，默认0
            output_tokens = getattr(getattr(response, 'usage', None), 'completion_tokens', 0)  # 安全获取输出token
            if not input_tokens and not output_tokens:  # 如果API未返回token数，则估算
                input_tokens = sum(len(m.content) for m in messages) // 2  # 粗略估算：每2字符≈1token
                output_tokens = len(getattr(response, 'content', '')) // 2
            ...
            CostTracker.record_usage(model_name, input_tokens, output_tokens, cost, str(uuid.uuid4()))  # 记录到成本追踪器
            ...
            try:  # 嵌套try，持久化失败不影响追踪
                ...
                if not company_id:  # 无企业ID时默认使用1
                    company_id = 1
                TokenUsagePersistence.record(TokenUsageRecord(  # 持久化到数据库
    ...
    def check_budget_before_call(self, messages: list[BaseMessage],
                                  rag_context: str = "",
    ...
    async def ainvoke_with_budget(self, messages: list[BaseMessage],
                                   rag_context: str = "",
    ...
    async def ainvoke_structured(self, messages: list[BaseMessage],
                                  output_schema: dict,
                                  **kwargs) -> dict:
        """结构化输出调用 - 4级兜底机制
    ...
    def get_stats(self) -> dict:
        ...
        return {
            ...
            "estimated_savings": round(
                self._cache_read_tokens * 0.0001, 4  # 粗略估算节省成本
            ),
        }

    @staticmethod
    def optimize_prompt_order(system_prompt: str, tool_definitions: str,
                               rag_context: str, user_prompt: str) -> str:
...
class StreamErrorHandler:
    ...
    def handle_user_cancelled(self, stream_state: StreamState):
        ...
        logger.info("stream_user_cancelled",
...
class ModelGateway:  # 模型网关核心类，集成配置管理、Key管理、配额管理、路由和模型创建
    ...
    def __init__(self, config_path: str = None):  # 配置文件路径可选，默认使用项目config目录下的model_config.yaml
        ...
        self.default_model = "deepseek_chat"  # 默认模型，在配置加载失败时使用
    ...
    def _load_config(self):  # 懒加载配置，仅在首次使用时加载
        ...
        try:  # 配置文件加载失败使用默认配置
            ...
            self.default_model = data.get('default_model', 'deepseek_chat')  # 提取默认模型
    ...
    def _default_config(self) -> dict:  # 硬编码的默认配置，确保在配置文件缺失时系统仍可运行
        return {
            "deepseek_chat": {  # DeepSeek聊天模型：性价比高，适合大多数场景
                "provider": "deepseek",
            ...
            "deepseek_reasoner": {  # DeepSeek推理模型：适合复杂推理任务，温度低以保证一致性
                ...
                "fallback_models": ["deepseek_chat"],  # 失败时降级为chat模型
            },
            "volcano_lite": {  # 火山引擎豆包Lite模型：成本最低，适合简单任务
                ...
                "suitable_for": ["simple"],
                "fallback_models": ["deepseek_chat"],  # 失败时降级为deepseek
            },
        }

    def _get_env_api_key(self, provider: str) -> str | None:  # 从环境变量获取API Key，映射规则可配置
        env_map = {  # 提供商到环境变量名的映射
            "deepseek": "DEEPSEEK_API_KEY",
            "openai": "OPENAI_API_KEY",
            "volcano": "VOLCANO_API_KEY",
        }
        env_key = env_map.get(provider, f"{provider.upper()}_API_KEY")  # 未知提供商使用通用命名规则
        return os.getenv(env_key)  # 返回None如果未设置

    def _create_model_instance(self, model_key: str,
                                company_api_key: str | None = None) -> ChatOpenAI:  # 创建ChatOpenAI实例，企业Key优先于环境变量
    ...
    def get_llm(self, model_key: str = None, company_api_key: str = None,
                company_id: int = None, agent_key: str = "") -> FailoverChatModel:  # 获取LLM实例，默认使用default_model
        ...
        if company_id and company_api_key is None:  # 指定了企业ID但未提供Key时自动获取
            company_api_key = self.key_manager.get_key(
                company_id,
                self.models_config.get(model_key or self.default_model, {}).get("provider", "deepseek"),
            )

        return FailoverChatModel(self, model_key or self.default_model,  # 使用默认模型作为兜底
                                  company_api_key, company_id, agent_key)

    def get_llm_for_agent(self, agent_name: str, task_description: str = "",
                          company_id: int = None, company_api_key: str = None) -> FailoverChatModel:  # 为Agent自动选择模型
        self._load_config()  # 确保配置已加载
