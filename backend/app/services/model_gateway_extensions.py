...
from sqlalchemy import text  # 用于构造 SQLAlchemy 文本 SQL，命名参数兼容 SQLite/PostgreSQL
...
class TokenUsagePersistence:  # 全部使用staticmethod，因为持久化操作不依赖实例状态，类似工具类
    ...
    def record(record: TokenUsageRecord):  # 记录单次模型调用的token用量到数据库
        try:  # 外层try包裹，确保数据库写入失败不影响主流程
            ...
            # 使用 engine.connect() + text() + 命名参数，与 cost_tracker.py 保持一致
            # PostgresAdapter 不再暴露 db.execute()，必须通过 engine 拿连接后执行
            ...
            with engine.connect() as conn:
                conn.execute(
                    text(
                        ...
                        "VALUES (:company_id, :agent_key, :model_name, :provider, "
                        ":input_tokens, :output_tokens, :cost_usd, :created_at)"
                    ),
                    {
                        "company_id": record.company_id,
                        "agent_key": record.agent_key
                        or "",  # 空字符串兜底，防止NULL值导致数据库约束问题
                        "model_name": record.model_name,
                        "provider": record.provider,
                        "input_tokens": record.input_tokens,
                        "output_tokens": record.output_tokens,
                        "cost_usd": record.cost_usd,
                        "created_at": datetime.utcnow(),  # 使用UTC时间，与数据库时区保持一致
                    },
                )
                conn.commit()
            ...
            logger.warning(
                "token_usage_persist_failed", error=str(e)
            )  # warning级别，因为不影响核心业务
        return False  # 失败返回False，调用方可以据此做降级处理

    @staticmethod  # 静态方法，查询操作不依赖实例状态
    def get_company_usage(
        company_id: int, days: int = 30
    ) -> list[dict]:  # 默认30天，符合常见报表周期
        try:
    ...
    def get_usage_summary(company_id: int, days: int = 30) -> dict:  # 汇总统计，返回聚合数据
        ...
        for r in records:
            ...
            by_agent[agent]["tokens"] += r.get("input_tokens", 0) + r.get(
                "output_tokens", 0
            )  # 总计token
...
class ModelHealthChecker:  # 模型健康检查器，全部使用classmethod，因为健康状态缓存是类级别的共享数据
    ...
    _status_cache: dict[
        str, "ModelHealthChecker.HealthStatus"
    ] = {}  # 类级别缓存，键为model_key；所有实例共享，避免重复检查

    @classmethod  # classmethod，需要使用gateway的配置信息
    async def check_model(
        cls, gateway, model_key: str
    ) -> HealthStatus:  # 异步方法，因为需要发送HTTP请求进行ping
    ...
    async def check_all(cls, gateway) -> dict:  # 异步遍历所有模型，并发检查
        ...
        for model_key in gateway.models_config:  # 遍历所有配置的模型
            results[model_key] = await cls.check_model(
                gateway, model_key
            )  # 逐个检查，注意：这里没有并发优化
        return {
            k: {  # 转换为字典格式，便于JSON序列化
...
class QuotaAlertService:  # 配额告警服务，全部使用classmethod，因为告警逻辑基于类级别阈值
    ...
    def check_and_alert(
        cls, company_id: int, quota_manager
    ) -> list[dict]:  # 检查配额使用率并生成告警
        alerts = []
