...
Level 1: Confidence >= 90% -> Silent auto-send (no human review)
...
"""  # 客服消息发送策略引擎：根据置信度和情绪风险分级处理，平衡自动化效率与人工审核安全
...
class SendLevel(StrEnum):  # 发送级别枚举，决定了消息是否需要人工介入
    SILENT = "silent"  # 静默发送：高置信度消息直接发送，无需人工确认
...
class SendDecision:  # 每次消息发送的决策结果，将策略评估结果与执行指令绑定
    ...
    auto_send: bool = False  # 默认不自动发送，仅SILENT级别的消息会设为True
    batch_group: str = ""  # 批次分组键，为空表示不归入批次
    created_at: str = field(default_factory=lambda: datetime.now().isoformat())  # 使用field避免可变默认值，时间戳用于审计


class SendStrategyEngine:  # 发送策略引擎，全部使用classmethod，因为策略评估是纯函数式的，不需要实例状态
    """Three-level message dispatch strategy engine."""

    CONFIDENCE_THRESHOLDS: dict[str, float] = {  # 类级别阈值，统一管理；使用float而非int，因为置信度是连续值
        "silent": 0.90,  # 90%以上置信度才静默发送，保守设置防止误发送
        "batch": 0.70,  # 70%以上进入批量审核，低于70%必须逐条审核
    }

    SENTIMENT_KEYWORDS: dict[SentimentRisk, list[str]] = {  # 按风险等级分组的情绪关键词，从高到低优先级匹配
    ...
    def evaluate(cls, message_id: str, customer_message: str, confidence: float,
                 agent_reply: str = "", context: dict = None) -> SendDecision:  # context可选，为未来扩展预留
        sentiment_risk = cls.detect_sentiment(customer_message)  # 先检测情绪风险，因为敏感话题检查优先级最高

        if cls._contains_sensitive_topic(customer_message) or cls._contains_sensitive_topic(agent_reply):  # 同时检查客户消息和回复，防止AI回复中无意涉及敏感话题
        ...
        if sentiment_risk in (SentimentRisk.CRITICAL, SentimentRisk.HIGH):  # 高/严重风险走REVIEW流程，不信任自动回复
            return SendDecision(
                message_id=message_id,
                ...
                escalation_note="高风险客户，建议暂停自动回复并交由人工处理" if sentiment_risk == SentimentRisk.CRITICAL
                else "不满客户，需人工审核回复内容",
            )

        if confidence >= cls.CONFIDENCE_THRESHOLDS["silent"]:  # 高置信度 + 低风险 → 静默发送
            return SendDecision(
                message_id=message_id,
                level=SendLevel.SILENT,
                confidence=confidence,
                reason=f"置信度{confidence*100:.0f}%，静默自动发送",
                sentiment_risk=sentiment_risk,
                auto_send=True,  # 标记为自动发送，调度器据此跳过人工审核
            )
    ...
    def detect_sentiment(cls, message: str) -> SentimentRisk:  # 基于关键词的情绪检测，从高到低优先级匹配
        message_lower = message.lower()  # 转小写进行不区分大小写的匹配

        for keyword in cls.SENTIMENT_KEYWORDS[SentimentRisk.CRITICAL]:  # 先检查严重风险，因为优先级最高
    ...
    def _contains_sensitive_topic(cls, text: str) -> bool:  # 下划线前缀表示内部方法，不对外暴露
        return any(topic in text.lower() for topic in [t.lower() for t in cls.SENSITIVE_TOPICS])  # 同时对话题和文本做lower，确保不区分大小写

    @classmethod  # classmethod，动作映射基于类级别配置
    def get_escalation_action(cls, risk: SentimentRisk) -> dict:  # 根据风险等级返回对应的升级动作配置
        actions = {  # 字典映射，O(1)查找，比if-elif链更高效且易于扩展
            ...
            SentimentRisk.CRITICAL: {  # 严重风险：仅人工处理，通知更多角色
                ...
                "notify_agents": ["customer_service", "brand_bd"],  # 同时通知BD，因为可能涉及品牌危机
            },
        }
        return actions.get(risk, actions[SentimentRisk.LOW])  # 兜底返回LOW级别的动作，防止未知风险等级导致错误
