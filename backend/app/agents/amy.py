...
v2.1.0 (2026-08-10): 高风险外发/退款/补偿改为默认草稿与人工确认
...
## 三级审核策略
- **置信度 ≥90%（快速审核草稿）**：生成标准回复草稿并标记低风险，仍需用户或客服确认后才可外发
- **置信度 70%-89%（批量确认）**：每小时汇总推送到审核列表，批量确认
- **置信度 <70%（逐条审核）**：暂停自动发送，即时推送人工审核
- 任何消息外发、退款、补偿、退货审核、批量运营动作都只能生成待审核草稿或建议；未得到显式人工确认前，不得声称已经发送、登记、退款、补偿或执行。
...
关于退货退款：
1. 我可以先为您整理退货退款申请信息，并提交人工售后审核
2. 待客服确认订单和售后政策后，会告知退回地址、运费承担方式和退款时效
3. 在人工确认前，我不会替您承诺已登记、已退款或已补偿

我也会把您反馈的产品批次、症状和订单信息整理为质检待办，交由人工客服确认后同步品控团队。您的健康是我们最关心的，再次向您致歉。

（此回复由AI客服生成，待人工确认后才会对外发送；如有需要可随时要求转接人工客服）\"""
"""

CUSTOMER_SERVICE_CAPABILITIES: list[
    str
...
CUSTOMER_SERVICE_DEFAULT_SKILLS: list[
    str
