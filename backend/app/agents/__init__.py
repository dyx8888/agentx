...
def get_active_agents() -> dict[str, dict]:
    """Return agent registry entries exposed through catalog endpoints.

    Entries can opt out by setting ``active=False``. Existing entries without
    the flag remain active for backward compatibility.
    """
    return {key: info for key, info in AGENT_REGISTRY.items() if info.get("active", True)}
