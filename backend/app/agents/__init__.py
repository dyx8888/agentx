...
Scope 对齐说明（docs/Scope.md S-OUT-04 / S-OUT-10）：
- product_selector / visual_designer / smart_ad_delivery 三个 agent 代码保留（S-OUT-10），
  但在当前阶段（第一期）标记为 active=False，不进入 Master 路由候选集，
  避免违背 S-OUT-04 对这三类能力的明确排除；第二三期再启用。
...
AGENT_REGISTRY: dict[
    str, dict
] = {  # 集中注册表——所有 agent 元数据单点维护，新增 agent 只需在此添加条目
    "master": {
    ...
    # ↓↓↓ S-OUT-04 排除项：代码保留（S-OUT-10）但 active=False，第二三期再启用 ↓↓↓
    "visual_designer": {
        ...
        "role": "design",
        "review_level": "recommended",
        "active": False,  # S-OUT-04 排除：第一期不激活，Master 不路由
    },
    "product_selector": {
...
def get_active_agents() -> dict[str, dict]:
    """返回当前阶段激活的 agent 子集（active=True）。

    Master 路由层应基于此结果选择候选 agent，避免路由到 S-OUT-04 排除的
    product_selector / visual_designer / smart_ad_delivery。
    代码保留但未激活的 agent 仍可通过 get_agent_definition 直接调用（用于调试/二期预研）。
    """
    return {k: v for k, v in AGENT_REGISTRY.items() if v.get("active", True) is True}
...
def get_agent_definition(agent_key: str) -> dict | None:
    ...
    try:
        ...
        logger.warning("agent_definition_load_failed", agent_key=agent_key, error=str(exc))
        return None  # 单个 agent 加载失败不应影响其他 agent 的可用性
...
def create_agent_execution_context(
    ...
    from app.agents.tools import (
        get_agent_tools,  # 延迟导入避免循环依赖——tools 模块会引用 agent 常量
    )
    ...
    context = {
        ...
        or {},  # 用 or 而非 if 判断，确保 company_context 始终是 dict
    }

    try:
        from app.rag.company_context_bus import (
            CompanyContextBus,  # 延迟导入 RAG 模块——不是所有场景都需要 RAG 上下文
        )

        bus = CompanyContextBus(
            str(company_id)
        )  # 将 company_id 转为字符串，因为 RAG 索引以字符串为 key
        rag_context = bus.get_context_for_agent(agent_key)
        if rag_context:
            context["rag_context"] = rag_context  # 只在有 RAG 上下文时才注入，避免空字段污染
    except Exception as exc:
        logger.warning(
            "agent_rag_context_load_failed",
            agent_key=agent_key,
            company_id=company_id,
            error=str(exc),
        )  # RAG 服务不可用时不阻塞主流程，Agent 仍可基于 system_prompt 工作
...
def load_engine_for_agent(agent_key: str):
    ...
    engine_map = {  # 只有需要确定性引擎的 agent 才映射——brand_bd/cc 这类纯 LLM agent 不需要引擎
        ...
        "warehouse_logistics": (
            "app.engines.warehouse_logistics_engine",
            "WarehouseLogisticsEngine",
        ),
...
def get_agent_collaboration_rules(agent_key: str) -> dict:
    ...
    return collab_rules.get(
        agent_key, {"can_delegate_to": [], "receives_from": []}
    )  # 默认空列表确保调用方安全遍历
...
def get_agent_engine_config(agent_key: str) -> dict:
    ...
    return configs.get(
        agent_key, {"preferred_model": "deepseek-chat", "max_tokens": 4096, "temperature": 0.7}
    )
