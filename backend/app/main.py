...
app.add_middleware(
    ...
    # 开发阶段允许所有来源，生产环境应限制为具体域名以防止 CSRF 攻击
    allow_credentials=True,  # 允许携带 Cookie/Authorization header，但 allow_origins=["*"] 时浏览器会忽略 credentials
...
async def root():
    ...
    return {
        ...
        "endpoints": {  # 手动维护端点列表作为快速参考，因为 Swagger /docs 需要浏览器访问，CLI 场景不友好
            "chat": "/chat",
            "feedback": "/feedback",
            "health": "/health",
            "docs": "/docs",
...
async def options_handler(path: str):
    ...
    return JSONResponse(
        status_code=200,  # OPTIONS 预检成功返回 200，浏览器收到后才会发送真正的 GET/POST 请求
        headers={
...
if __name__ == "__main__":  # 仅当直接运行此文件时启动 uvicorn，被 gunicorn import 时不执行，避免端口冲突
    uvicorn.run(
        ...
        reload=True,  # 开发环境启用热重载，代码修改后自动重启，生产环境必须关闭
