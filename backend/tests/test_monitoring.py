...
class TestMonitoring:
    ...
    async def test_2_request_count_increments(self):
        ...
        with TestClient(app) as client:
            ...
            # 解析初始请求数（对所有 label 组合求和，因为 /health 和 /metrics 是不同 label）
            initial_count = sum(
                int(m.group(1))
                for m in re.finditer(r"agentx_requests_total\{[^}]*\} ([0-9]+)", initial_content)
            )
            ...
            # 解析更新后请求数（对所有 label 组合求和）
            updated_count = sum(
                int(m.group(1))
                for m in re.finditer(r"agentx_requests_total\{[^}]*\} ([0-9]+)", updated_content)
            )
    ...
    async def test_4_middleware_no_impact(self):
        ...
        with TestClient(app) as client:
            # 发送认证请求（即使返回 401）
            client.post("/auth/token", json={"username": "testuser", "password": "wrongpassword"})
    ...
    async def test_6_metrics_format_valid(self):
        ...
        with TestClient(app) as client:
            ...
            # 样本行（非注释行）应包含 Prometheus 格式的花括号
            sample_lines = [line for line in lines if not line.startswith("#") and line.strip()]
            assert len(sample_lines) > 0
            has_braces = any("{" in line and "}" in line for line in sample_lines)
            assert has_braces, "样本行应包含 Prometheus 标签花括号"
