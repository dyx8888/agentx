...
class TestPlatformCredentials:
    ...
    def setup_method(self):
        ...
        # 创建无凭证测试公司
        no_cred_company = type('TestCompany', (), {
    ...
    def test_1_company_specific_credentials_loading(self):
        ...
        test_credentials = {
            "douyin_star": {
                "api_key": "test_company_key",
                "api_secret": "test_company_secret"
            }
        }

        # Mock 公司有凭证
        test_company_with_creds = type('TestCompany', (), {
            'id': self.test_company_id,
            'name': 'Test Company',
            'brand_name': 'Test Brand',
            'category': 'beauty',
        ...
        db.get_company = lambda company_id: test_company_with_creds if company_id == self.test_company_id else None

        # 创建适配器并验证凭证
        adapter = DouyinStarAdapter(company_id=self.test_company_id)

        assert adapter.api_key == "test_company_key", f"Expected test_company_key, got {adapter.api_key}"
        assert adapter.api_secret == "test_company_secret", f"Expected test_company_secret, got {adapter.api_secret}"
    ...
    def test_2_fallback_to_environment_variables(self):
        ...
        try:
            ...
            # 验证降级到环境变量
            assert adapter.api_key == 'env_test_key', f"Expected env_test_key, got {adapter.api_key}"
            assert adapter.api_secret == 'env_test_secret', f"Expected env_test_secret, got {adapter.api_secret}"
    ...
    def test_3_bind_credentials_api_permission_denied(self):
        ...
        # Mock 认证依赖
        import app.api.companies
        import app.auth
        original_get_current_user = app.auth.get_current_active_user

        def mock_get_current_user():
            return mock_user

        app.auth.get_current_active_user = mock_get_current_user
        ...
        try:
            ...
            response = self.client.post(
                f"/admin/companies/{self.test_company_id}/credentials",
            ...
            # 恢复原始认证依赖
            app.api.companies.get_current_active_user = original_get_current_user
    ...
    def test_4_unbind_nonexistent_platform(self):
        ...
        try:
            ...
            response = self.client.delete(
                f"/admin/companies/{self.test_company_id}/credentials?platform=nonexistent_platform"
            )
