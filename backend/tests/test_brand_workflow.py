...
class TestBrandWorkflow:
    ...
    def test_1_load_workflow_template_success(self):
        ...
        try:
            ...
            # 验证模板名称
            assert template["name"] == "brand_bd_standard", (
                f"Expected 'brand_bd_standard', got {template['name']}"
            )
    ...
    def test_2_execute_brand_bd_workflow_success(self):
        ...
        # Mock 数据库操作
        with (
            patch("app.database.db.create_workflow") as mock_create_workflow,
            patch("app.database.db.create_a2a_message") as mock_create_message,
            patch("app.database.db.update_a2a_message_status"),
            patch("app.database.db.update_workflow_status"),
        ):
            # Mock 数据库返回工作流ID
            ...
            with patch("app.workflow.engine.get_agent_for_tools") as mock_get_agent:
                ...
                # 调用品牌商务工作流执行
                result = self.engine.execute_brand_bd_workflow(
                    self.test_company_id, self.test_context
                )

                # 验证返回的工作流ID不为空
                assert result is not None, "Workflow ID should not be None"
                assert result != "Error executing brand BD workflow", (
                ...
                assert call_args[0][0] == self.test_company_id, "Company ID should match"
                assert call_args[0][1] == "brand_bd_standard", "Workflow name should match"

                # 验证模板变量替换
                definition_json = call_args[0][2]
    ...
    def test_3_load_nonexistent_template_error(self):
        ...
        assert "non_existent_template" in error_message, (
            "Error message should mention template name"
        )
    ...
    def test_4_execute_with_missing_context_params(self):
        ...
        with (
            ...
            with patch("app.workflow.engine.get_agent_for_tools") as mock_get_agent:
                ...
                result = self.engine.execute_brand_bd_workflow(
                    self.test_company_id, incomplete_context
                )
                ...
                definition_json = mock_create_workflow.call_args[0][2]
                assert "beauty" in definition_json, "Category should be replaced"
                assert "测试产品" in definition_json, "Product name should be replaced"
