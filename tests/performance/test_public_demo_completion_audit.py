...
def _fake_git(
    ...
    def run(args):
        ...
        if key == ("rev-list", "-n", "1", "public-demo-local-20260811-v3"):
        ...
        if key == ("ls-remote", "--tags", "origin", "public-demo-local-20260811-v3"):
            if remote_pushed:
                return GitResult(0, f"{tag_target}\trefs/tags/public-demo-local-20260811-v3", "")
...
def test_completion_audit_passes_local_ready_with_external_pending():
    report = run_audit(
        ...
        baseline_tag="public-demo-local-20260811-v3",
        git=_fake_git(),
        read_text=_reader(),
    )
